                         ...
                         // CheckPoint(0)
{
  ...
  reset_iteration_timing();
  total_iter_timing = current_iter_timing;
  completed_iterations = 0;
  total_timing_printed = false;
#endif
  ...
  Waveshell = new surface_integral(Symmetry);
    ...
    // Initialize shared memory interpolation for load balancing
    {
        int max_nn_total = 20 * 96 * 96 * 4; // conservative upper bound
        int max_num_var = 17;
        InterpSHM *interp_shm_ptr = new InterpSHM();
        interp_shm_ptr->init(max_nn_total, max_num_var);
        Waveshell->set_interp_shm(interp_shm_ptr);
    }
...
void bssn_class::Initialize()
{
  if (myrank == 0)
  {
    cout << " [TRACE] Initialize(): begin setup with " << ngfs << " grid functions." << endl;
  ...
  char pname[50];
  {
    ...
    if (iter != parameters::str_par.end())
    {
      ...
      if (myrank == 0)
        cout << " [TRACE] Initialize(): using parameter file " << pname << " for grid composition." << endl;
    }
    else
    {
      cout << "Error inputpar" << endl;
  ...
  if (myrank == 0)
    cout << " [TRACE] Initialize(): cgh constructed; checkrun=" << checkrun << endl;
  if (checkrun)
  {
    if (myrank == 0)
      cout << " [TRACE] Initialize(): reading checkpointed grid hierarchy." << endl;
    CheckPoint->readcheck_cgh(PhysTime, GH, myrank, nprocs, Symmetry);
  }
  else
  {
    if (myrank == 0)
      cout << " [TRACE] Initialize(): composing grid hierarchy from scratch." << endl;
    GH->compose_cgh(nprocs);
  }
#ifdef WithShell
  SH = new ShellPatch(0, ngfs, pname, Symmetry, myrank, ErrorMonitor);
  if (myrank == 0)
    cout << " [TRACE] Initialize(): shell patch constructed." << endl;
  ...
  {
    if (myrank == 0)
      cout << " [TRACE] Initialize(): loading shell checkpoint." << endl;
  ...
  dT = Courant * h;
  if (myrank == 0)
    cout << " [TRACE] Initialize(): base spacing h=" << h << ", Courant=" << Courant << ", dT=" << dT << endl;
  ...
  if (checkrun)
  {
    ...
    if (myrank == 0)
      cout << " [TRACE] Initialize(): restored puncture data from checkpoint." << endl;
  }
  else
  {
    PhysTime = StartTime;
    ...
    if (myrank == 0)
      cout << " [TRACE] Initialize(): Setup_Black_Hole_position() completed." << endl;
  }
}
...
void bssn_class::reset_iteration_timing()
{
#ifdef BSSN_TIMING_ENABLED
  BSSN_FIELD_SET(iteration_wall, 0.0);
  BSSN_FIELD_SET(evolve_kernel, 0.0);
  BSSN_FIELD_SET(analysis, 0.0);
  BSSN_FIELD_SET(constraint, 0.0);
  BSSN_FIELD_SET(dump_binary, 0.0);
  current_iter_timing.dump_2d = 0.0;
  BSSN_FIELD_SET(checkpoint, 0.0);
  BSSN_FIELD_SET(regrid, 0.0);
  BSSN_FIELD_SET(memory_probe, 0.0);
  BSSN_FIELD_SET(abort_check, 0.0);
  BSSN_FIELD_SET(kernel_step, 0.0);
  BSSN_FIELD_SET(kernel_enforce, 0.0);
  BSSN_FIELD_SET(kernel_restrict, 0.0);
  BSSN_FIELD_SET(kernel_restrict_prepare, 0.0);
  BSSN_FIELD_SET(kernel_restrict_restrict, 0.0);
  BSSN_FIELD_SET(kernel_restrict_sync_coarse, 0.0);
  BSSN_FIELD_SET(kernel_restrict_outbd, 0.0);
  BSSN_FIELD_SET(kernel_restrict_sync_fine, 0.0);
  BSSN_FIELD_SET(kernel_regrid_local, 0.0);
  BSSN_FIELD_SET(kernel_step_rhs, 0.0);
  BSSN_FIELD_SET(kernel_step_rhs_predictor, 0.0);
  BSSN_FIELD_SET(kernel_step_rhs_corrector, 0.0);
  BSSN_FIELD_SET(kernel_step_rungekutta, 0.0);
  BSSN_FIELD_SET(kernel_step_boundary, 0.0);
  BSSN_FIELD_SET(kernel_step_sync, 0.0);
  BSSN_FIELD_SET(kernel_step_shell_rhs, 0.0);
  BSSN_FIELD_SET(kernel_step_shell_rungekutta, 0.0);
  BSSN_FIELD_SET(kernel_step_shell_boundary, 0.0);
  BSSN_FIELD_SET(kernel_step_shell_sync, 0.0);
  BSSN_FIELD_SET(kernel_step_bh, 0.0);
  BSSN_FIELD_SET(kernel_step_analysis, 0.0);
  BSSN_FIELD_SET(kernel_step_analysis_waveshell, 0.0);
  BSSN_FIELD_SET(kernel_step_analysis_fderivs, 0.0);
  current_iter_timing.kernel_step_analysis_psi4 = 0.0;
  BSSN_FIELD_SET(kernel_step_analysis_waveshell_surf_wave, 0.0);
  BSSN_FIELD_SET(kernel_step_analysis_waveshell_surf_masspang, 0.0);
  BSSN_FIELD_SET(kernel_step_analysis_waveshell_interp, 0.0);
  BSSN_FIELD_SET(kernel_step_analysis_waveshell_int, 0.0);
  BSSN_FIELD_SET(kernel_step_analysis_waveshell_comm, 0.0);
  BSSN_FIELD_SET(kernel_step_analysis_waveshell_prepare, 0.0);
  BSSN_FIELD_SET(kernel_step_enforce_ga, 0.0);
  BSSN_FIELD_SET(kernel_step_lowerbound, 0.0);
  BSSN_FIELD_SET(kernel_step_error_check, 0.0);
  BSSN_FIELD_SET(kernel_step_loop_overhead, 0.0);
#endif  // BSSN_TIMING_ENABLED
}

void bssn_class::finalize_iteration_timing(int stepIndex, double physicalTime, bool isTerminal, const char *reason)
{
#ifdef BSSN_TIMING_ENABLED
  total_iter_timing.iteration_wall += current_iter_timing.iteration_wall;
  total_iter_timing.evolve_kernel += current_iter_timing.evolve_kernel;
  total_iter_timing.analysis += current_iter_timing.analysis;
  total_iter_timing.constraint += current_iter_timing.constraint;
  total_iter_timing.dump_binary += current_iter_timing.dump_binary;
  total_iter_timing.dump_2d += current_iter_timing.dump_2d;
  total_iter_timing.checkpoint += current_iter_timing.checkpoint;
  total_iter_timing.regrid += current_iter_timing.regrid;
  total_iter_timing.memory_probe += current_iter_timing.memory_probe;
  total_iter_timing.abort_check += current_iter_timing.abort_check;
  total_iter_timing.kernel_step += current_iter_timing.kernel_step;
  total_iter_timing.kernel_enforce += current_iter_timing.kernel_enforce;
  total_iter_timing.kernel_restrict += current_iter_timing.kernel_restrict;
  total_iter_timing.kernel_restrict_prepare += current_iter_timing.kernel_restrict_prepare;
  total_iter_timing.kernel_restrict_restrict += current_iter_timing.kernel_restrict_restrict;
  total_iter_timing.kernel_restrict_sync_coarse += current_iter_timing.kernel_restrict_sync_coarse;
  total_iter_timing.kernel_restrict_outbd += current_iter_timing.kernel_restrict_outbd;
  total_iter_timing.kernel_restrict_sync_fine += current_iter_timing.kernel_restrict_sync_fine;
  total_iter_timing.kernel_regrid_local += current_iter_timing.kernel_regrid_local;
  total_iter_timing.kernel_step_rhs += current_iter_timing.kernel_step_rhs;
  total_iter_timing.kernel_step_rhs_predictor += current_iter_timing.kernel_step_rhs_predictor;
  total_iter_timing.kernel_step_rhs_corrector += current_iter_timing.kernel_step_rhs_corrector;
  total_iter_timing.kernel_step_rungekutta += current_iter_timing.kernel_step_rungekutta;
  total_iter_timing.kernel_step_boundary += current_iter_timing.kernel_step_boundary;
  total_iter_timing.kernel_step_sync += current_iter_timing.kernel_step_sync;
  total_iter_timing.kernel_step_shell_rhs += current_iter_timing.kernel_step_shell_rhs;
  total_iter_timing.kernel_step_shell_rungekutta += current_iter_timing.kernel_step_shell_rungekutta;
  total_iter_timing.kernel_step_shell_boundary += current_iter_timing.kernel_step_shell_boundary;
  total_iter_timing.kernel_step_shell_sync += current_iter_timing.kernel_step_shell_sync;
  total_iter_timing.kernel_step_bh += current_iter_timing.kernel_step_bh;
  total_iter_timing.kernel_step_analysis += current_iter_timing.kernel_step_analysis;
  // Accumulate detailed timing
  total_iter_timing.kernel_step_analysis_waveshell += current_iter_timing.kernel_step_analysis_waveshell; 
  total_iter_timing.kernel_step_analysis_fderivs += current_iter_timing.kernel_step_analysis_fderivs;
  total_iter_timing.kernel_step_analysis_psi4 += current_iter_timing.kernel_step_analysis_psi4;
  total_iter_timing.kernel_step_analysis_waveshell_surf_wave += current_iter_timing.kernel_step_analysis_waveshell_surf_wave;
  total_iter_timing.kernel_step_analysis_waveshell_surf_masspang += current_iter_timing.kernel_step_analysis_waveshell_surf_masspang;
  total_iter_timing.kernel_step_analysis_waveshell_interp += current_iter_timing.kernel_step_analysis_waveshell_interp;
  total_iter_timing.kernel_step_analysis_waveshell_int += current_iter_timing.kernel_step_analysis_waveshell_int;
  total_iter_timing.kernel_step_analysis_waveshell_comm += current_iter_timing.kernel_step_analysis_waveshell_comm;
  total_iter_timing.kernel_step_analysis_waveshell_prepare += current_iter_timing.kernel_step_analysis_waveshell_prepare;
  total_iter_timing.kernel_step_enforce_ga += current_iter_timing.kernel_step_enforce_ga;
  total_iter_timing.kernel_step_lowerbound += current_iter_timing.kernel_step_lowerbound;
  total_iter_timing.kernel_step_error_check += current_iter_timing.kernel_step_error_check;
  total_iter_timing.kernel_step_loop_overhead += current_iter_timing.kernel_step_loop_overhead;
  ...
  if (myrank == 0)
  {
    ...
    cout << setprecision(6);
    cout << " [ITER TRACE] step " << stepIndex << " t=" << physicalTime
      << " wall=" << current_iter_timing.iteration_wall << " s" << endl;
    cout << "   kernel=" << current_iter_timing.evolve_kernel
      << " s analysis=" << current_iter_timing.analysis
      << " s constraint=" << current_iter_timing.constraint << " s" << endl;
    cout << "   dump(binary)=" << current_iter_timing.dump_binary
      << " s dump(2d)=" << current_iter_timing.dump_2d
      << " s checkpoint=" << current_iter_timing.checkpoint << " s" << endl;
    cout << "   regrid=" << current_iter_timing.regrid
      << " s memory=" << current_iter_timing.memory_probe
      << " s abort_check=" << current_iter_timing.abort_check << " s" << endl;
    double kernel_breakdown_sum = current_iter_timing.kernel_step
      + current_iter_timing.kernel_enforce
      + current_iter_timing.kernel_restrict
      + current_iter_timing.kernel_regrid_local;
    double kernel_other = current_iter_timing.evolve_kernel - kernel_breakdown_sum;
    if (kernel_other < 0.0)
      kernel_other = 0.0;
    cout << "   kernel detail: step=" << current_iter_timing.kernel_step
      << " s enforce=" << current_iter_timing.kernel_enforce
      << " s restrict=" << current_iter_timing.kernel_restrict
      << " s regrid_local=" << current_iter_timing.kernel_regrid_local
      << " s other=" << kernel_other << " s" << endl;
    if (current_iter_timing.kernel_restrict > 0.0) {
      double restrict_sub_sum = current_iter_timing.kernel_restrict_prepare
        + current_iter_timing.kernel_restrict_restrict
        + current_iter_timing.kernel_restrict_sync_coarse
        + current_iter_timing.kernel_restrict_outbd
        + current_iter_timing.kernel_restrict_sync_fine;
      double restrict_other = current_iter_timing.kernel_restrict - restrict_sub_sum;
      if (restrict_other < 0.0) restrict_other = 0.0;
      cout << "     restrict detail: prepare=" << current_iter_timing.kernel_restrict_prepare
        << " s restrict=" << current_iter_timing.kernel_restrict_restrict
        << " s sync_coarse=" << current_iter_timing.kernel_restrict_sync_coarse
        << " s outbd=" << current_iter_timing.kernel_restrict_outbd
        << " s sync_fine=" << current_iter_timing.kernel_restrict_sync_fine
        << " s other=" << restrict_other << " s" << endl;
    }
    double step_patch_sum = current_iter_timing.kernel_step_rhs
      + current_iter_timing.kernel_step_rungekutta
      + current_iter_timing.kernel_step_boundary
      + current_iter_timing.kernel_step_sync
      + current_iter_timing.kernel_step_bh
      + current_iter_timing.kernel_step_analysis
      + current_iter_timing.kernel_step_enforce_ga
      + current_iter_timing.kernel_step_lowerbound
      + current_iter_timing.kernel_step_error_check
      + current_iter_timing.kernel_step_loop_overhead;
    double step_shell_sum = current_iter_timing.kernel_step_shell_rhs
      + current_iter_timing.kernel_step_shell_rungekutta
      + current_iter_timing.kernel_step_shell_boundary
      + current_iter_timing.kernel_step_shell_sync;
    double step_other = current_iter_timing.kernel_step - (step_patch_sum + step_shell_sum);
    if (step_other < 0.0)
      step_other = 0.0;
    cout << "   step detail (patch): rhs=" << current_iter_timing.kernel_step_rhs
      << " [pre=" << current_iter_timing.kernel_step_rhs_predictor << " cor=" << current_iter_timing.kernel_step_rhs_corrector << "]"
      << " s rk=" << current_iter_timing.kernel_step_rungekutta
      << " s boundary=" << current_iter_timing.kernel_step_boundary
      << " s sync=" << current_iter_timing.kernel_step_sync
      << " s bh=" << current_iter_timing.kernel_step_bh
      << " s analysis=" << current_iter_timing.kernel_step_analysis
      << " s other=" << step_other << " s" << endl;
    // Show breakdown of previously unmeasured operations
    if (current_iter_timing.kernel_step_enforce_ga > 0.0 || current_iter_timing.kernel_step_lowerbound > 0.0 ||
        current_iter_timing.kernel_step_error_check > 0.0 || current_iter_timing.kernel_step_loop_overhead > 0.0) {
      cout << "     step overhead: enforce_ga=" << current_iter_timing.kernel_step_enforce_ga
           << " s lowerbound=" << current_iter_timing.kernel_step_lowerbound
           << " s error_check=" << current_iter_timing.kernel_step_error_check
           << " s loop_overhead=" << current_iter_timing.kernel_step_loop_overhead << " s" << endl;
    }
    if (current_iter_timing.kernel_step_analysis > 0.0) {
      cout << "     analysis detail: waveshell=" << current_iter_timing.kernel_step_analysis_waveshell
           << " s fderivs=" << current_iter_timing.kernel_step_analysis_fderivs
           << " s psi4=" << current_iter_timing.kernel_step_analysis_psi4
           << " s other=" << (current_iter_timing.kernel_step_analysis - current_iter_timing.kernel_step_analysis_waveshell - current_iter_timing.kernel_step_analysis_fderivs - current_iter_timing.kernel_step_analysis_psi4) << " s" << endl;
      if (current_iter_timing.kernel_step_analysis_waveshell > 0.0) {
        cout << "       waveshell breakup: surf_wave=" << current_iter_timing.kernel_step_analysis_waveshell_surf_wave
             << " s surf_mass=" << current_iter_timing.kernel_step_analysis_waveshell_surf_masspang
             << " s | interp=" << current_iter_timing.kernel_step_analysis_waveshell_interp
             << " s int=" << current_iter_timing.kernel_step_analysis_waveshell_int
             << " s comm=" << current_iter_timing.kernel_step_analysis_waveshell_comm
             << " s prep=" << current_iter_timing.kernel_step_analysis_waveshell_prepare << " s" << endl;
      }
    }
    if (step_shell_sum > 0.0)
    {
      cout << "   step detail (shell): rhs=" << current_iter_timing.kernel_step_shell_rhs
        << " s rk=" << current_iter_timing.kernel_step_shell_rungekutta
        << " s boundary=" << current_iter_timing.kernel_step_shell_boundary
        << " s sync=" << current_iter_timing.kernel_step_shell_sync << " s" << endl;
    }
    cout.flags(oldFlags);
  ...
  if (isTerminal)
  {
    print_total_timing_summary(reason);
  }
#else
  (void)stepIndex; (void)physicalTime; (void)isTerminal; (void)reason;
#endif  // BSSN_TIMING_ENABLED
}

void bssn_class::print_total_timing_summary(const char *reason)
{
#ifdef BSSN_TIMING_ENABLED
  if (myrank != 0 || total_timing_printed)
    return;
  ...
  cout << " [TIMING SUMMARY] iterations=" << completed_iterations;
  if (reason && reason[0] != '\0')
    cout << " trigger=" << reason;
  cout << endl;

  cout << "   totals(s): wall=" << total_iter_timing.iteration_wall
    << " kernel=" << total_iter_timing.evolve_kernel
    << " analysis=" << total_iter_timing.analysis
    << " constraint=" << total_iter_timing.constraint << endl;
  cout << "              dump(binary)=" << total_iter_timing.dump_binary
    << " dump(2d)=" << total_iter_timing.dump_2d
    << " checkpoint=" << total_iter_timing.checkpoint << endl;
  cout << "              regrid=" << total_iter_timing.regrid
    << " memory_probe=" << total_iter_timing.memory_probe
    << " abort_check=" << total_iter_timing.abort_check << endl;
  double total_kernel_breakdown_sum = total_iter_timing.kernel_step
    + total_iter_timing.kernel_enforce
    + total_iter_timing.kernel_restrict
    + total_iter_timing.kernel_regrid_local;
  double total_kernel_other = total_iter_timing.evolve_kernel - total_kernel_breakdown_sum;
  if (total_kernel_other < 0.0)
    total_kernel_other = 0.0;
  cout << "              kernel detail totals: step=" << total_iter_timing.kernel_step
    << " enforce=" << total_iter_timing.kernel_enforce
    << " restrict=" << total_iter_timing.kernel_restrict
    << " regrid_local=" << total_iter_timing.kernel_regrid_local
    << " other=" << total_kernel_other << endl;
  if (total_iter_timing.kernel_restrict > 0.0) {
    double r_sub_sum = total_iter_timing.kernel_restrict_prepare
      + total_iter_timing.kernel_restrict_restrict
      + total_iter_timing.kernel_restrict_sync_coarse
      + total_iter_timing.kernel_restrict_outbd
      + total_iter_timing.kernel_restrict_sync_fine;
    double r_other = total_iter_timing.kernel_restrict - r_sub_sum;
    if (r_other < 0.0) r_other = 0.0;
    cout << "                restrict totals: prepare=" << total_iter_timing.kernel_restrict_prepare
      << " restrict=" << total_iter_timing.kernel_restrict_restrict
      << " sync_coarse=" << total_iter_timing.kernel_restrict_sync_coarse
      << " outbd=" << total_iter_timing.kernel_restrict_outbd
      << " sync_fine=" << total_iter_timing.kernel_restrict_sync_fine
      << " other=" << r_other << endl;
  }

  if (completed_iterations > 0)
  {
    double denom = static_cast<double>(completed_iterations);
    cout << "   average(s): wall=" << total_iter_timing.iteration_wall / denom
      << " kernel=" << total_iter_timing.evolve_kernel / denom
      << " analysis=" << total_iter_timing.analysis / denom
      << " constraint=" << total_iter_timing.constraint / denom << endl;
    cout << "              dump(binary)=" << total_iter_timing.dump_binary / denom
      << " dump(2d)=" << total_iter_timing.dump_2d / denom
      << " checkpoint=" << total_iter_timing.checkpoint / denom << endl;
    cout << "              regrid=" << total_iter_timing.regrid / denom
      << " memory_probe=" << total_iter_timing.memory_probe / denom
      << " abort_check=" << total_iter_timing.abort_check / denom << endl;
    double avg_kernel_breakdown_sum = (total_iter_timing.kernel_step
      + total_iter_timing.kernel_enforce
      + total_iter_timing.kernel_restrict
      + total_iter_timing.kernel_regrid_local) / denom;
    double avg_kernel_step = total_iter_timing.kernel_step / denom;
    double avg_kernel_enforce = total_iter_timing.kernel_enforce / denom;
    double avg_kernel_restrict = total_iter_timing.kernel_restrict / denom;
    double avg_kernel_regrid_local = total_iter_timing.kernel_regrid_local / denom;
    double avg_kernel_other = (total_iter_timing.evolve_kernel / denom) - avg_kernel_breakdown_sum;
    if (avg_kernel_other < 0.0)
      avg_kernel_other = 0.0;
    cout << "              kernel detail avg: step=" << avg_kernel_step
      << " enforce=" << avg_kernel_enforce
      << " restrict=" << avg_kernel_restrict
      << " regrid_local=" << avg_kernel_regrid_local
      << " other=" << avg_kernel_other << endl;
    if (avg_kernel_restrict > 0.0) {
      double r_sub_avg = (total_iter_timing.kernel_restrict_prepare
        + total_iter_timing.kernel_restrict_restrict
        + total_iter_timing.kernel_restrict_sync_coarse
        + total_iter_timing.kernel_restrict_outbd
        + total_iter_timing.kernel_restrict_sync_fine) / denom;
      double r_other_avg = avg_kernel_restrict - r_sub_avg;
      if (r_other_avg < 0.0) r_other_avg = 0.0;
      cout << "                restrict avg: prepare=" << (total_iter_timing.kernel_restrict_prepare / denom)
        << " restrict=" << (total_iter_timing.kernel_restrict_restrict / denom)
        << " sync_coarse=" << (total_iter_timing.kernel_restrict_sync_coarse / denom)
        << " outbd=" << (total_iter_timing.kernel_restrict_outbd / denom)
        << " sync_fine=" << (total_iter_timing.kernel_restrict_sync_fine / denom)
        << " other=" << r_other_avg << endl;
    }
    double avg_step_patch_sum = (total_iter_timing.kernel_step_rhs
      + total_iter_timing.kernel_step_rungekutta
      + total_iter_timing.kernel_step_boundary
      + total_iter_timing.kernel_step_sync
      + total_iter_timing.kernel_step_bh
      + total_iter_timing.kernel_step_analysis) / denom;
    double avg_step_shell_sum = (total_iter_timing.kernel_step_shell_rhs
      + total_iter_timing.kernel_step_shell_rungekutta
      + total_iter_timing.kernel_step_shell_boundary
      + total_iter_timing.kernel_step_shell_sync) / denom;
    double avg_step_other = avg_kernel_step - (avg_step_patch_sum + avg_step_shell_sum);
    if (avg_step_other < 0.0)
      avg_step_other = 0.0;
    cout << "              step detail avg (patch): rhs=" << (total_iter_timing.kernel_step_rhs / denom)
      << " rk=" << (total_iter_timing.kernel_step_rungekutta / denom)
      << " boundary=" << (total_iter_timing.kernel_step_boundary / denom)
      << " sync=" << (total_iter_timing.kernel_step_sync / denom)
      << " bh=" << (total_iter_timing.kernel_step_bh / denom)
      << " analysis=" << (total_iter_timing.kernel_step_analysis / denom)
      << " other=" << avg_step_other << endl;
    double avg_shell_rhs = total_iter_timing.kernel_step_shell_rhs / denom;
    double avg_shell_rk = total_iter_timing.kernel_step_shell_rungekutta / denom;
    double avg_shell_boundary = total_iter_timing.kernel_step_shell_boundary / denom;
    double avg_shell_sync = total_iter_timing.kernel_step_shell_sync / denom;
    double avg_shell_total = avg_shell_rhs + avg_shell_rk + avg_shell_boundary + avg_shell_sync;
    if (avg_shell_total > 0.0)
    {
      cout << "              step detail avg (shell): rhs=" << avg_shell_rhs
        << " rk=" << avg_shell_rk
        << " boundary=" << avg_shell_boundary
        << " sync=" << avg_shell_sync << endl;
    }
  }

  cout.flags(oldFlags);
  cout.precision(oldPrecision);
  total_timing_printed = true;
#endif  // BSSN_TIMING_ENABLED
}
...
//initial checkpoint for special uasge
  ...
  if (myrank == 0)
    cout << " [TRACE] Starting initial constraint interpolation." << endl;
  ...
  if (myrank == 0)
    cout << " [TRACE] Initial constraint interpolation complete." << endl;

  if (checkrun)
  {
    if (myrank == 0)
      cout << " [TRACE] Restart detected: loading checkpointed BSSN state." << endl;
    CheckPoint->read_bssn(LastDump, Last2dDump, LastAnas);
  }

  if (!checkrun && myrank == 0)
    cout << " [TRACE] Fresh evolution state will advance from current initial data." << endl;
  ...
  */
  ...
  if (myrank == 0)
  {
    cout << " [TRACE] Entering bssn_class::Evolve with Steps=" << Steps
         << ", StartTime=" << StartTime
         << ", PhysTime=" << PhysTime
         << ", TotalTime=" << TotalTime << endl;
    cout << " [TRACE] Control parameters: DumpTime=" << DumpTime
         << ", d2DumpTime=" << d2DumpTime
         << ", CheckTime=" << CheckTime
         << ", AnalysisTime=" << AnasTime
         << ", dT(mon)=" << dT_mon
         << ", trfls=" << trfls << endl;
    cout << " [TRACE] Mode flags: checkrun=" << checkrun
         << ", Symmetry=" << Symmetry
         << ", PSTR=" << PSTR
         << ", REGLEV=" << REGLEV
         << ", RPS=" << RPS << endl;
  ...
  {
    // 多线程下不应该用 clock() 计时，应该用 MPI_Wtime()
    prev_clock = MPI_Wtime();

    reset_iteration_timing();
    double iter_start_wall = MPI_Wtime();
...
#if (PSTR == 0)
    {
    BSSN_SCOPED_TRACE("evolve_kernel");
    double kernel_start = MPI_Wtime();
    RecursiveStep(0);
    BSSN_FIELD_ACCUM(evolve_kernel, MPI_Wtime() - kernel_start);
    }
#elif (PSTR == 1 || PSTR == 2 || PSTR == 3)
  // data analysis part
  // Warning NOTE: the variables1 are used as temp storage room
    {
    BSSN_SCOPED_TRACE("analysis");
    double analysis_start = MPI_Wtime();
    AnalysisStuff(a_lev, dT_mon);
    BSSN_FIELD_ACCUM(analysis, MPI_Wtime() - analysis_start);
    }
    ...
    {
    BSSN_SCOPED_TRACE("constraint");
    double constraint_start = MPI_Wtime();
    Constraint_Out(); // this will affect the Dump_List
    BSSN_FIELD_ACCUM(constraint, MPI_Wtime() - constraint_start);
    }
    ...
    if (LastDump >= DumpTime)
    {
      ...
      LastDump = 0;
      BSSN_FIELD_ACCUM(dump_binary, MPI_Wtime() - dump_start);
      }
    ...
    {
      {
      BSSN_SCOPED_TRACE("dump_2d");
      double dump2d_start = MPI_Wtime();
      ...
      Last2dDump = 0;
      BSSN_FIELD_ACCUM(dump_2d, MPI_Wtime() - dump2d_start);
      }
    ...
    {
      BSSN_FIELD_SET(iteration_wall, MPI_Wtime() - iter_start_wall);
      finalize_iteration_timing(ncount, PhysTime, true, "Reached TotalTime");
...
#if (REGLEV == 1)
    {
    BSSN_SCOPED_TRACE("regrid");
    double regrid_start = MPI_Wtime();
    GH->Regrid(Symmetry, BH_num, Porgbr, Porg0,
               SynchList_cor, OldStateList, StateList, SynchList_pre,
               fgt(PhysTime - dT_mon, StartTime, dT_mon / 2), ErrorMonitor);
    BSSN_FIELD_ACCUM(regrid, MPI_Wtime() - regrid_start);
    }
#endif
...
//		fgt(PhysTime-dT_mon,StartTime,dT_mon/2),ErrorMonitor);
    ...
    {
    BSSN_SCOPED_TRACE("memory_probe");
    double memory_start = MPI_Wtime();
    bssn_perf.MemoryUsage(&current_min, &current_avg, &current_max,
                &peak_min, &peak_avg, &peak_max, nprocs);
    BSSN_FIELD_ACCUM(memory_probe, MPI_Wtime() - memory_start);
    }
    if (myrank == 0)
    {
    ...
    ////////////////////////////////////////////////////////////
    // 如果检测到有输入 "stop"，则退出 MPI 程序
    // If an "stop" command is detected on stdin, terminate MPI processes
    // XiaoQu
    ////////////////////////////////////////////////////////////
    ...
    {
      {
      BSSN_SCOPED_TRACE("abort_check");
      double abort_start = MPI_Wtime();
      if (check_Stdin_Stop())
      {
        shouldAbort = true;
      }
      BSSN_FIELD_ACCUM(abort_check, MPI_Wtime() - abort_start);
      }
    }
    ...
    if (abortFlag)
    {
      ...
      {
        BSSN_FIELD_SET(iteration_wall, MPI_Wtime() - iter_start_wall);
        finalize_iteration_timing(ncount, PhysTime, true, "User stop request");
        iteration_finalized = true;
      }
      if (myrank == 0)
      {
        cout << endl;
        cout << " Aborting the MPI process in response to 'stop' command." << endl;
        cout << endl;
      }
      MPI_Abort(MPI_COMM_WORLD, 1);
    }
    ...
    {
      {
      BSSN_SCOPED_TRACE("checkpoint");
      double checkpoint_start = MPI_Wtime();
      LastCheck = 0;
...
#endif
      CheckPoint->write_bssn(LastDump, Last2dDump, LastAnas);
      BSSN_FIELD_ACCUM(checkpoint, MPI_Wtime() - checkpoint_start);
      }
    }
    ...
    {
      BSSN_FIELD_SET(iteration_wall, MPI_Wtime() - iter_start_wall);
      finalize_iteration_timing(ncount, PhysTime, false, 0);
      iteration_finalized = true;
    }
  }

  if (!total_timing_printed && completed_iterations > 0)
    print_total_timing_summary("Completed evolution loop");
  /*
  #ifdef With_AHF
...
// 该成员函数设定了时间演化过程中的不同层网格间递归时间演化（仅仅针对 PSTR == 0 的情况）
...
void bssn_class::RecursiveStep(int lev)
{
  ...
  for (int i = 0; i < NoIterations; i++)
  {
    ...
    // Step(lev, YN);
    // BSSN_FIELD_ACCUM(kernel_step, MPI_Wtime() - step_start);
    BSSN_TIMED("kernel_step", kernel_step, Step(lev, YN));

#if (AGM == 2)
    if (GH->levels == 1)
    {
      {
      BSSN_SCOPED_TRACE("kernel_enforce");
      double enforce_start = MPI_Wtime();
      Enforce_algcon(lev, 0);
      BSSN_FIELD_ACCUM(kernel_enforce, MPI_Wtime() - enforce_start);
      }
    }
#endif
    ...
    {
    BSSN_SCOPED_TRACE("kernel_restrict");
    double restrict_start = MPI_Wtime();
    RestrictProlong(lev, YN, fgt(PhysTime - dT_lev, StartTime, dT_lev / 2), StateList, OldStateList, SynchList_cor);
    BSSN_FIELD_ACCUM(kernel_restrict, MPI_Wtime() - restrict_start);
    }
...
#if (REGLEV == 0)
  {
  BSSN_SCOPED_TRACE("kernel_regrid_local");
  double regrid_local_start = MPI_Wtime();
  GH->Regrid_Onelevel(lev, Symmetry, BH_num, Porgbr, Porg0,
                      SynchList_cor, OldStateList, StateList, SynchList_pre,
                      fgt(PhysTime - dT_lev, StartTime, dT_lev / 2), ErrorMonitor);
  BSSN_FIELD_ACCUM(kernel_regrid_local, MPI_Wtime() - regrid_local_start);
  }
#endif
}
...
// 该成员函数设定了时间演化过程中的不同层网格间递归时间演化（针对 PSTR == 1 和 PSTR == 2 的情况）
// This member function implements recursive time-stepping across AMR levels
...
// 该成员函数设定了时间演化过程中的不同层网格间的时间演化（递归方式）
// This member function implements recursive time-stepping across AMR levels
...
// 该成员函数设定了时间演化过程中的每层网格的单步时间演化（针对 PSTR == 0 的情形）
...
// new code 2013-2-15, zjcao
  ...
  {
    {
    BSSN_SCOPED_TRACE("kernel_step_bh");
    double bh_start = MPI_Wtime();
    ...
    BSSN_FIELD_ACCUM(kernel_step_bh, MPI_Wtime() - bh_start);
    }
  }
  ...
  {
    {
    BSSN_SCOPED_TRACE("kernel_step_analysis");
    double analysis_start = MPI_Wtime();
    AnalysisStuff(lev, dT_lev);
    BSSN_FIELD_ACCUM(kernel_step_analysis, MPI_Wtime() - analysis_start);
    }
  }
#endif
  ...
  // Pre-build arrays from linked lists for parallel variable loops
  int nvar = 0;
  { MyList<var> *tmp = StateList; while (tmp) { nvar++; tmp = tmp->next; } }
  int *sgfn0 = new int[nvar];
  int *sgfn_pre = new int[nvar];
  int *sgfn_cor = new int[nvar];
  int *sgfn_rhs = new int[nvar];
  double *propspeed_arr = new double[nvar];
  double *SoA_arr = new double[nvar * 3];
  {
    MyList<var> *v0=StateList, *vp=SynchList_pre, *vc=SynchList_cor, *vr=RHSList;
    for (int v = 0; v < nvar; v++) {
      sgfn0[v] = v0->data->sgfn;
      sgfn_pre[v] = vp->data->sgfn;
      sgfn_cor[v] = vc->data->sgfn;
      sgfn_rhs[v] = vr->data->sgfn;
      propspeed_arr[v] = v0->data->propspeed;
      for (int d = 0; d < 3; d++) SoA_arr[v*3+d] = v0->data->SoA[d];
      v0=v0->next; vp=vp->next; vc=vc->next; vr=vr->next;
    }
  }
  ...
  while (Pp)
  {
    ...
    while (BP)
    {
      ...
      if (myrank == cg->rank)
      {
#if (AGM == 0)
        {
        BSSN_SCOPED_TRACE("kernel_step_enforce_ga");
        double enforce_ga_start = MPI_Wtime();
        f_enforce_ga(cg->shape,
                     cg->fgfs[gxx0->sgfn], cg->fgfs[gxy0->sgfn], cg->fgfs[gxz0->sgfn],
                     cg->fgfs[gyy0->sgfn], cg->fgfs[gyz0->sgfn], cg->fgfs[gzz0->sgfn],
                     cg->fgfs[Axx0->sgfn], cg->fgfs[Axy0->sgfn], cg->fgfs[Axz0->sgfn],
                     cg->fgfs[Ayy0->sgfn], cg->fgfs[Ayz0->sgfn], cg->fgfs[Azz0->sgfn]);
        BSSN_FIELD_ACCUM(kernel_step_enforce_ga, MPI_Wtime() - enforce_ga_start);
        }
#endif

        {
        BSSN_SCOPED_TRACE("kernel_step_rhs_predictor");
        double rhs_start = MPI_Wtime();
        ...
        double rhs_duration = MPI_Wtime() - rhs_start;
        // printf("[Profile] shape: %d %d %d, rhs_duration: %lf\n", cg->shape[0], cg->shape[1], cg->shape[2], rhs_duration);
        BSSN_FIELD_ACCUM(kernel_step_rhs, rhs_duration);
        BSSN_FIELD_ACCUM(kernel_step_rhs_predictor, rhs_duration);
        }


        // rk4 substep and boundary (parallelized over variables)
        {
#if (SommerType == 0)
          ...
          for (int v = 0; v < nvar; v++)
          {
            ...
            if (lev > 0) // fix BD point
              ...
              f_sommerfeld_rout(cg->shape, cg->X[0], cg->X[1], cg->X[2],
                                ...
                                cg->fgfs[phi0->sgfn],
                                cg->fgfs[Lap0->sgfn],
                                cg->fgfs[sgfn0[v]], cg->fgfs[sgfn_pre[v]],
                                SoA_arr + v*3,
                                Symmetry, cor);
          }
#endif
...
#warning "shell part still bam type"
        ...
        {
        BSSN_SCOPED_TRACE("kernel_step_lowerbound");
        double lowerbound_start = MPI_Wtime();
        f_lowerboundset(cg->shape, cg->fgfs[phi->sgfn], chitiny);
        BSSN_FIELD_ACCUM(kernel_step_lowerbound, MPI_Wtime() - lowerbound_start);
        }
      }
      if (BP == Pp->data->ble)
        break;
  ...
  // check error information (disabled for production)
  // {
  //   {
  //   BSSN_SCOPED_TRACE("kernel_step_error_check");
  //   double error_check_start = MPI_Wtime();
  //   int erh = ERROR;
  //   MPI_Allreduce(&erh, &ERROR, 1, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
  //   BSSN_FIELD_ACCUM(kernel_step_error_check, MPI_Wtime() - error_check_start);
  //   }
  // }
  // if (ERROR)
  // {
    ...
    while (sPp)
    {
      ...
      while (BP)
      {
        ...
        if (myrank == cg->rank)
        {
          ...
          {
          BSSN_SCOPED_TRACE("kernel_step_shell_rhs");
          double rhs_start = MPI_Wtime();
          ...
          BSSN_FIELD_ACCUM(kernel_step_shell_rhs, MPI_Wtime() - rhs_start);
          }
          ...
          // rk4 substep and boundary
          {
            ...
            while (varl0)
            {
              ...
              {
              BSSN_SCOPED_TRACE("kernel_step_shell_boundary");
              ...
              f_sommerfeld_routbam_ss(cg->shape, cg->X[0], cg->X[1], cg->X[2],
                                      ...
                                      Symmetry);
              BSSN_FIELD_ACCUM(kernel_step_shell_boundary, MPI_Wtime() - boundary_start);
              }

              {
              BSSN_SCOPED_TRACE("kernel_step_shell_rungekutta");
              double rk_start = MPI_Wtime();
              f_rungekutta4_rout(cg->shape, dT_lev, 
                                 cg->fgfs[varl0->data->sgfn], 
                                 cg->fgfs[varl->data->sgfn], 
                                 cg->fgfs[varlrhs->data->sgfn],
                                 iter_count);
              BSSN_FIELD_ACCUM(kernel_step_shell_rungekutta, MPI_Wtime() - rk_start);
              }
...
// check rhs    
  ...
  {
    {
    BSSN_SCOPED_TRACE("kernel_step_sync");
    double sync_start = MPI_Wtime();
    Parallel::Sync(GH->PatL[lev], SynchList_pre, Symmetry);
    BSSN_FIELD_ACCUM(kernel_step_sync, MPI_Wtime() - sync_start);
    }
  }
  ...
  if (lev == 0)
  {
    ...
    {
    BSSN_SCOPED_TRACE("kernel_step_shell_sync");
    double shell_sync_start = MPI_Wtime();
    SH->Synch(SynchList_pre, Symmetry);
    BSSN_FIELD_ACCUM(kernel_step_shell_sync, MPI_Wtime() - shell_sync_start);
    }
  ...
  {
    {
    BSSN_SCOPED_TRACE("kernel_step_loop_overhead");
    double corrector_iter_start = MPI_Wtime();
    // for RK4: t0, t0+dt/2, t0+dt/2, t0+dt;
    if (iter_count == 1 || iter_count == 3)
      TRK4 += dT_lev / 2;
    BSSN_FIELD_ACCUM(kernel_step_loop_overhead, MPI_Wtime() - corrector_iter_start);
    }
    Pp = GH->PatL[lev];
    while (Pp)
    {
      ...
      while (BP)
      {
        ...
        if (myrank == cg->rank)
        {
          ...
          {
          BSSN_SCOPED_TRACE("kernel_step_rhs_corrector");
          double rhs_start = MPI_Wtime();
          if (f_compute_rhs_bssn(cg->shape, TRK4, cg->X[0], cg->X[1], cg->X[2],
          ...
          double rhs_duration = MPI_Wtime() - rhs_start;
          BSSN_FIELD_ACCUM(kernel_step_rhs, rhs_duration);
          BSSN_FIELD_ACCUM(kernel_step_rhs_corrector, rhs_duration);
          }
          // rk4 substep and boundary (parallelized over variables)
          {
#if (SommerType == 0)
            ...
            for (int v = 0; v < nvar; v++)
            {
              ...
              if (lev > 0) // fix BD point
                ...
                f_sommerfeld_rout(cg->shape, cg->X[0], cg->X[1], cg->X[2],
                                  ...
                                  cg->fgfs[phi0->sgfn],
                                  cg->fgfs[Lap0->sgfn],
                                  cg->fgfs[sgfn0[v]], cg->fgfs[sgfn_cor[v]],
                                  SoA_arr + v*3,
                                  Symmetry, cor);
            }
#endif
            ...
            for (int v = 0; v < nvar; v++)
            {
              ...
              if (lev == 1) // shibata type sommerfeld
                f_sommerfeld_rout(cg->shape, cg->X[0], cg->X[1], cg->X[2],
                                  ...
                                  cg->fgfs[phi0->sgfn],
                                  cg->fgfs[Lap0->sgfn],
                                  cg->fgfs[sgfn_pre[v]], cg->fgfs[sgfn_cor[v]],
                                  SoA_arr + v*3,
                                  Symmetry, cor);
            }
#endif
  ...
  {
    {
    BSSN_SCOPED_TRACE("kernel_step_boundary");
    double boundary_start = MPI_Wtime();
    RestrictProlong(lev, YN, BB);
    BSSN_FIELD_ACCUM(kernel_step_boundary, MPI_Wtime() - boundary_start);
    }
  }
...
// 该成员函数设定了时间演化过程中的每层网格的单步时间演化（针对 PSTR == 0 以外的情形）
...
//  a_stream.setf(ios::left);
  ...
  {
    MyList<Patch> *Pp, *Ppc;
    double _t; // sub-timer for RestrictProlong phases
    if (lev > trfls && YN == 0) // time refinement levels and for intermediat time level
    {
      // --- phase: prepare_inter_time_level ---
      {
      BSSN_SCOPED_TRACE("kernel_restrict_prepare");
      _t = MPI_Wtime();
      Pp = GH->PatL[lev - 1];
      while (Pp)
      {
...
//	 Pp->data->checkPatch(0,GH->start_rank[GH->mylev]);
      ...
      BSSN_FIELD_ACCUM(kernel_restrict_prepare, MPI_Wtime() - _t);
      }
...
//       misc::tillherecheck(GH->Commlev[GH->mylev],GH->start_rank[GH->mylev],a_stream.str());
      ...
      // --- phase: Restrict ---
      {
      BSSN_SCOPED_TRACE("kernel_restrict_restrict");
      _t = MPI_Wtime();
#if (RPB == 0)
...
#endif
      BSSN_FIELD_ACCUM(kernel_restrict_restrict, MPI_Wtime() - _t);
      }


      // --- phase: Sync coarse level ---
      {
      BSSN_SCOPED_TRACE("kernel_restrict_sync_coarse");
      _t = MPI_Wtime();
      Parallel::Sync(GH->PatL[lev - 1], SynchList_pre, Symmetry);
      BSSN_FIELD_ACCUM(kernel_restrict_sync_coarse, MPI_Wtime() - _t);
      }

      // --- phase: OutBdLow2Hi ---
      {
      BSSN_SCOPED_TRACE("kernel_restrict_outbd");
      _t = MPI_Wtime();
#if (RPB == 0)
      Ppc = GH->PatL[lev - 1];
      while (Ppc)
...
#endif
      BSSN_FIELD_ACCUM(kernel_restrict_outbd, MPI_Wtime() - _t);
      }
    }
...
//       misc::tillherecheck(GH->Commlev[GH->mylev],GH->start_rank[GH->mylev],a_stream.str());
    ...
    // --- phase: Sync fine level ---
    {
    BSSN_SCOPED_TRACE("kernel_restrict_sync_fine");
    _t = MPI_Wtime();
    Parallel::Sync(GH->PatL[lev], SL, Symmetry);
    BSSN_FIELD_ACCUM(kernel_restrict_sync_fine, MPI_Wtime() - _t);
    }
...
// 该成员函数用于计算引力波和准局域质量等相关数据
// This member function computes gravitational-wave and quasi-local related quantities and performs analysis
...
#error "not support parallel levels yet"
...
#else
    {
    BSSN_SCOPED_TRACE("kernel_step_analysis_psi4");
    double psi4_start = MPI_Wtime();
    Compute_Psi4(lev);
    BSSN_FIELD_ACCUM(kernel_step_analysis_psi4, MPI_Wtime() - psi4_start);
    }
#endif
    ...
    double Rex = maxrex;
    {
    BSSN_SCOPED_TRACE("kernel_step_analysis_waveshell");
    double waveshell_start = MPI_Wtime();
    Waveshell->reset_timer();
    for (int i = 0; i < decn; i++)
    {
#ifdef Point_Psi4
      { BSSN_SCOPED_TRACE("waveshell_surf_wave");
      ...
      {
        BSSN_SCOPED_TRACE("waveshell_surf_masspang");
...
#if (PSTR == 0)
      {
        // Batched: handle all radii in one Interp_Points call
        double *Rex_array = new double[decn];
        for (int ri = 0; ri < decn; ri++) Rex_array[ri] = maxrex - ri * drex;

        double *RP_all = new double[NN * decn];
        double *IP_all = new double[NN * decn];
        double *RoutMAP_all = new double[7 * decn];

        { BSSN_SCOPED_TRACE("waveshell_surf_wave");
          Waveshell->surf_Wave_Batch(Rex_array, decn, lev, GH,
                                     Rpsi4, Ipsi4, 2, maxl, NN,
                                     RP_all, IP_all, ErrorMonitor);
        }
        { BSSN_SCOPED_TRACE("waveshell_surf_masspang");
          Waveshell->surf_MassPAng_Batch(Rex_array, decn, lev, GH, phi0, trK0,
                                         gxx0, gxy0, gxz0, gyy0, gyz0, gzz0,
                                         Axx0, Axy0, Axz0, Ayy0, Ayz0, Azz0,
                                         Gmx0, Gmy0, Gmz0, Sfx1, Sfy1, Sfz1,
                                         RoutMAP_all, ErrorMonitor);
        }

        ADMMass = RoutMAP_all[0]; // mass from first (outermost) radius
        { BSSN_SCOPED_TRACE("waveshell_writefile");
          for (int ri = 0; ri < decn; ri++)
          {
            Psi4Monitor->writefile(PhysTime, NN, RP_all + ri * NN, IP_all + ri * NN);
            MAPMonitor->writefile(PhysTime, 7, RoutMAP_all + ri * 7);
          }
        }
        ...
        delete[] IP_all;
        delete[] RoutMAP_all;
        break; // all radii handled, exit decn loop
      }
#elif (PSTR == 1 || PSTR == 2)
      { BSSN_SCOPED_TRACE("waveshell_surf_wave");
...
//        misc::tillherecheck(GH->Commlev[lev],GH->start_rank[lev],"end surface integral");
...
#if (PSTR == 1 || PSTR == 2)
      { BSSN_SCOPED_TRACE("waveshell_mpi_gather");
    ...
    BSSN_FIELD_ACCUM(kernel_step_analysis_waveshell, MPI_Wtime() - waveshell_start);
    }
    BSSN_FIELD_ACCUM(kernel_step_analysis_waveshell_surf_wave, Waveshell->time_surf_wave);
    BSSN_FIELD_ACCUM(kernel_step_analysis_waveshell_surf_masspang, Waveshell->time_surf_masspang);
    BSSN_FIELD_ACCUM(kernel_step_analysis_waveshell_interp, Waveshell->time_interp);
    BSSN_FIELD_ACCUM(kernel_step_analysis_waveshell_int, Waveshell->time_integration);
    BSSN_FIELD_ACCUM(kernel_step_analysis_waveshell_comm, Waveshell->time_communication);
    BSSN_FIELD_ACCUM(kernel_step_analysis_waveshell_prepare, Waveshell->time_prepare);
    delete[] RP;
    delete[] IP;
    delete[] RoutMAP;
...
void bssn_class::AH_Step_Find(int lev, double dT_lev)
{
  if ((lev == GH->levels - 1))
  {
    ...
    if (tf)
    {
      ...
      if (myrank == 0)
      {
        cout << " Finding horizon used " << curr_clock - prev_clock << " seconds! " << endl;
      }
    }
  }
}
...
// This member function monitors stdin for an 'stop' input
