...
!$OMP PARALLEL PRIVATE(i,j,k,ts0_thr,tid)
...
! set kmax and kmin 0
  ...
  enddo
!$OMP END DO
  call sft_trace("fderivs", ts0_thr, sft_get_ts(), 0, tid)
!$OMP END PARALLEL
...
! set imax and imin 0
  ...
  enddo
!$OMP END DO
  call sft_trace("fdx", ts0_thr, sft_get_ts(), 0, tid)
!$OMP END PARALLEL
...
! set jmax and jmin 0
  ...
  enddo
!$OMP END DO
  call sft_trace("fdy", ts0_thr, sft_get_ts(), 0, tid)
!$OMP END PARALLEL
...
! set kmax and kmin 0
  ...
  enddo
!$OMP END DO
  call sft_trace("fdz", ts0_thr, sft_get_ts(), 0, tid)
!$OMP END PARALLEL
...
!~~~~~~ fyz
   ...
   enddo
!$OMP END DO
  call sft_trace("fdderivs", ts0_thr, sft_get_ts(), 0, tid)
!$OMP END PARALLEL
...
! 2nd-order _inner: caller pre-fills fh via symmetry_bd_inner(1,...) and
! manages !$OMP PARALLEL barriers. Mirrors the regular fderivs/fdderivs.
! ---------------------------------------------------------------------------
  subroutine fderivs_inner(ex,fh,fx,fy,fz,X,Y,Z,symmetry)
  use sft_trace_mod
  use omp_lib
  implicit none
  integer(8) :: ts0_thr
  ...
  real*8,  dimension(0:ex(1),0:ex(2),0:ex(3)), intent(in) :: fh
  real*8,  dimension(ex(1),ex(2),ex(3)), intent(out):: fx,fy,fz
  real*8,                                intent(in) :: X(ex(1)),Y(ex(2)),Z(ex(3))
...
!$OMP DO COLLAPSE(1)
  ...
  do j=1,ex(2)
  do i=1,ex(1)
    if(i<ex(1) .and. j<ex(2) .and. k<ex(3))then
      if(i+1 <= imax .and. i-1 >= imin)then
        fx(i,j,k)=d2dx*(-fh(i-1,j,k)+fh(i+1,j,k))
      else
        fx(i,j,k) = ZEO
  ...
  enddo
!$OMP END DO NOWAIT
  call sft_trace("fderivs_inner", ts0_thr, sft_get_ts(), 0, tid)

  return

  end subroutine fderivs_inner

  subroutine fdderivs_inner(ex,fh,fxx,fxy,fxz,fyy,fyz,fzz,X,Y,Z,symmetry)
  use sft_trace_mod
  use omp_lib
  implicit none
  integer(8) :: ts0_thr
...
!$OMP DO COLLAPSE(1)
  ...
  enddo
!$OMP END DO NOWAIT
  call sft_trace("fdderivs_inner", ts0_thr, sft_get_ts(), 0, tid)

  return

  end subroutine fdderivs_inner
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!~~~~~~ fxx
   ...
   enddo
!$OMP END DO
  call sft_trace("fddxx", ts0_thr, sft_get_ts(), 0, tid)
!$OMP END PARALLEL
...
!~~~~~~ fyy
   ...
   enddo
!$OMP END DO
  call sft_trace("fddyy", ts0_thr, sft_get_ts(), 0, tid)
!$OMP END PARALLEL
...
!~~~~~~ fzz
   ...
   enddo
!$OMP END DO
  call sft_trace("fddzz", ts0_thr, sft_get_ts(), 0, tid)
!$OMP END PARALLEL
...
!~~~~~~ fxy
   ...
   enddo
!$OMP END DO
  call sft_trace("fddxy", ts0_thr, sft_get_ts(), 0, tid)
!$OMP END PARALLEL
...
!~~~~~~ fxz
   ...
   enddo
!$OMP END DO
  call sft_trace("fddxz", ts0_thr, sft_get_ts(), 0, tid)
!$OMP END PARALLEL
...
!~~~~~~ fyz
   ...
   enddo
!$OMP END DO
  call sft_trace("fddyz", ts0_thr, sft_get_ts(), 0, tid)
!$OMP END PARALLEL
...
!~~~~~~ fyz
   ...
   enddo
!$OMP END DO
  call sft_trace("fdderivsdavid", ts0_thr, sft_get_ts(), 0, tid)
!$OMP END PARALLEL
...
!-----------------------------------------------------------------------------
! Inner version of fderivs: no OMP PARALLEL, no symmetry_bd
! fh is passed in pre-computed from the caller
!-----------------------------------------------------------------------------
  subroutine fderivs_inner(ex,fh,fx,fy,fz,X,Y,Z,symmetry)
  use sft_trace_mod
  use omp_lib
...
#if 0
! x direction
        if(i<ex(1) .and. j<ex(2) .and. k<ex(3) .and. &
           i+2 <= imax .and. i-2 >= imin)then
      fx(i,j,k)=d12dx*(fh(i-2,j,k)-EIT*fh(i-1,j,k)+EIT*fh(i+1,j,k)-fh(i+2,j,k))
    elseif(i<ex(1) .and. j<ex(2) .and. k<ex(3) .and. &
           i+1 <= imax .and. i-1 >= imin)then
      fx(i,j,k)=d2dx*(-fh(i-1,j,k)+fh(i+1,j,k))
    else
      fx(i,j,k) = ZEO
    endif
! y direction
        if(i<ex(1) .and. j<ex(2) .and. k<ex(3) .and. &
           j+2 <= jmax .and. j-2 >= jmin)then
      fy(i,j,k)=d12dy*(fh(i,j-2,k)-EIT*fh(i,j-1,k)+EIT*fh(i,j+1,k)-fh(i,j+2,k))
    elseif(i<ex(1) .and. j<ex(2) .and. k<ex(3) .and. &
           j+1 <= jmax .and. j-1 >= jmin)then
     fy(i,j,k)=d2dy*(-fh(i,j-1,k)+fh(i,j+1,k))
    else
      fy(i,j,k) = ZEO
    endif
...
! x direction
      ...
      fx(i,j,k)=d12dx*(fh(i-2,j,k)-EIT*fh(i-1,j,k)+EIT*fh(i+1,j,k)-fh(i+2,j,k))
    elseif(i<ex(1) .and. j<ex(2) .and. k<ex(3) .and. &
           i+3 <= imax .and. i-1 >= imin)then
      fx(i,j,k)=d12dx*(-3.d0*fh(i-1,j,k)-1.d1*fh(i,j,k)+1.8d1*fh(i+1,j,k)-6.d0*fh(i+2,j,k)+fh(i+3,j,k))
    elseif(i<ex(1) .and. j<ex(2) .and. k<ex(3) .and. &
           i+1 <= imax .and. i-3 >= imin)then
      fx(i,j,k)=d12dx*( 3.d0*fh(i+1,j,k)+1.d1*fh(i,j,k)-1.8d1*fh(i-1,j,k)+6.d0*fh(i-2,j,k)-fh(i-3,j,k))
    else
      fx(i,j,k) = ZEO
    endif
! y direction
      ...
      fy(i,j,k)=d12dy*(fh(i,j-2,k)-EIT*fh(i,j-1,k)+EIT*fh(i,j+1,k)-fh(i,j+2,k))
    elseif(i<ex(1) .and. j<ex(2) .and. k<ex(3) .and. &
           j+3 <= jmax .and. j-1 >= jmin)then
      fy(i,j,k)=d12dy*(-3.d0*fh(i,j-1,k)-1.d1*fh(i,j,k)+1.8d1*fh(i,j+1,k)-6.d0*fh(i,j+2,k)+fh(i,j+3,k))
    elseif(i<ex(1) .and. j<ex(2) .and. k<ex(3) .and. &
           j+1 <= jmax .and. j-3 >= jmin)then
      fy(i,j,k)=d12dy*( 3.d0*fh(i,j+1,k)+1.d1*fh(i,j,k)-1.8d1*fh(i,j-1,k)+6.d0*fh(i,j-2,k)-fh(i,j-3,k))
    else
      fy(i,j,k) = ZEO
    endif
! z direction
      ...
      fz(i,j,k)=d12dz*(fh(i,j,k-2)-EIT*fh(i,j,k-1)+EIT*fh(i,j,k+1)-fh(i,j,k+2))
    elseif(i<ex(1) .and. j<ex(2) .and. k<ex(3) .and. &
           k+3 <= kmax .and. k-1 >= kmin)then
      fz(i,j,k)=d12dz*(-3.d0*fh(i,j,k-1)-1.d1*fh(i,j,k)+1.8d1*fh(i,j,k+1)-6.d0*fh(i,j,k+2)+fh(i,j,k+3))
    elseif(i<ex(1) .and. j<ex(2) .and. k<ex(3) .and. &
           k+1 <= kmax .and. k-3 >= kmin)then
      fz(i,j,k)=d12dz*( 3.d0*fh(i,j,k+1)+1.d1*fh(i,j,k)-1.8d1*fh(i,j,k-1)+6.d0*fh(i,j,k-2)-fh(i,j,k-3))
    else
      fz(i,j,k) = ZEO
    endif
...
!-----------------------------------------------------------------------------
! Inner version of fdderivs: no OMP PARALLEL, no symmetry_bd
! fh is passed in pre-computed from the caller
!-----------------------------------------------------------------------------
  subroutine fdderivs_inner(ex,fh,fxx,fxy,fxz,fyy,fyz,fzz,X,Y,Z,symmetry)
  use sft_trace_mod
...
!~~~~~~ fxy
       if(i<ex(1) .and. j<ex(2) .and. k<ex(3) .and. &
          i+2 <= imax .and. i-2 >= imin .and. j+2 <= jmax .and. j-2 >= jmin)then
   fxy(i,j,k) = Fdxdy*(     (fh(i-2,j-2,k)-F8*fh(i-1,j-2,k)+F8*fh(i+1,j-2,k)-fh(i+2,j-2,k))  &
                       -F8 *(fh(i-2,j-1,k)-F8*fh(i-1,j-1,k)+F8*fh(i+1,j-1,k)-fh(i+2,j-1,k))  &
                       +F8 *(fh(i-2,j+1,k)-F8*fh(i-1,j+1,k)+F8*fh(i+1,j+1,k)-fh(i+2,j+1,k))  &
                       -    (fh(i-2,j+2,k)-F8*fh(i-1,j+2,k)+F8*fh(i+1,j+2,k)-fh(i+2,j+2,k)))
   elseif(i<ex(1) .and. j<ex(2) .and. k<ex(3) .and. &
          i+1 <= imax .and. i-1 >= imin .and. j+1 <= jmax .and. j-1 >= jmin)then
   fxy(i,j,k) = Sdxdy*(fh(i-1,j-1,k)-fh(i+1,j-1,k)-fh(i-1,j+1,k)+fh(i+1,j+1,k))
   else
   fxy(i,j,k) = ZEO
   endif
!~~~~~~ fxz
       if(i<ex(1) .and. j<ex(2) .and. k<ex(3) .and. &
          i+2 <= imax .and. i-2 >= imin .and. k+2 <= kmax .and. k-2 >= kmin)then
   fxz(i,j,k) = Fdxdz*(     (fh(i-2,j,k-2)-F8*fh(i-1,j,k-2)+F8*fh(i+1,j,k-2)-fh(i+2,j,k-2))  &
                       -F8 *(fh(i-2,j,k-1)-F8*fh(i-1,j,k-1)+F8*fh(i+1,j,k-1)-fh(i+2,j,k-1))  &
                       +F8 *(fh(i-2,j,k+1)-F8*fh(i-1,j,k+1)+F8*fh(i+1,j,k+1)-fh(i+2,j,k+1))  &
                       -    (fh(i-2,j,k+2)-F8*fh(i-1,j,k+2)+F8*fh(i+1,j,k+2)-fh(i+2,j,k+2)))
   elseif(i<ex(1) .and. j<ex(2) .and. k<ex(3) .and. &
          i+1 <= imax .and. i-1 >= imin .and. k+1 <= kmax .and. k-1 >= kmin)then
   fxz(i,j,k) = Sdxdz*(fh(i-1,j,k-1)-fh(i+1,j,k-1)-fh(i-1,j,k+1)+fh(i+1,j,k+1))
   else
   fxz(i,j,k) = ZEO
   endif
!~~~~~~ fyz
       if(i<ex(1) .and. j<ex(2) .and. k<ex(3) .and. &
          j+2 <= jmax .and. j-2 >= jmin .and. k+2 <= kmax .and. k-2 >= kmin)then
   fyz(i,j,k) = Fdydz*(     (fh(i,j-2,k-2)-F8*fh(i,j-1,k-2)+F8*fh(i,j+1,k-2)-fh(i,j+2,k-2))  &
                       -F8 *(fh(i,j-2,k-1)-F8*fh(i,j-1,k-1)+F8*fh(i,j+1,k-1)-fh(i,j+2,k-1))  &
                       +F8 *(fh(i,j-2,k+1)-F8*fh(i,j-1,k+1)+F8*fh(i,j+1,k+1)-fh(i,j+2,k+1))  &
                       -    (fh(i,j-2,k+2)-F8*fh(i,j-1,k+2)+F8*fh(i,j+1,k+2)-fh(i,j+2,k+2)))
   elseif(i<ex(1) .and. j<ex(2) .and. k<ex(3) .and. &
          j+1 <= jmax .and. j-1 >= jmin .and. k+1 <= kmax .and. k-1 >= kmin)then
   fyz(i,j,k) = Sdydz*(fh(i,j-1,k-1)-fh(i,j+1,k-1)-fh(i,j-1,k+1)+fh(i,j+1,k+1))
   else
   fyz(i,j,k) = ZEO
   endif
...
! ---------------------------------------------------------------------------
! Stubs for *_inner helpers used by compute_rhs_bssn_opt.
! These are dead code at runtime when Equation_Class != "BSSN", but the
! 6th-order _inner: caller pre-fills fh via symmetry_bd_inner(3,...) and
! manages !$OMP PARALLEL barriers. Mirrors the regular fderivs/fdderivs
! cascades above, with per-direction tier selection (6th -> 4th -> 2nd).
! ---------------------------------------------------------------------------
  subroutine fderivs_inner(ex,fh,fx,fy,fz,X,Y,Z,symmetry)
  use sft_trace_mod
  use omp_lib
...
! 8th-order _inner: caller pre-fills fh via symmetry_bd_inner(4,...) and
! manages !$OMP PARALLEL barriers. Mirrors the regular fderivs/fdderivs
! cascades above, with per-direction tier selection (8th -> 6th -> 4th -> 2nd).
! ---------------------------------------------------------------------------
  subroutine fderivs_inner(ex,fh,fx,fy,fz,X,Y,Z,symmetry)
  use sft_trace_mod
  use omp_lib
  ...
  integer,                               intent(in ):: ex(1:3),symmetry
  real*8,  dimension(-3:ex(1),-3:ex(2),-3:ex(3)), intent(in) :: fh
  real*8,  dimension(ex(1),ex(2),ex(3)), intent(out):: fx,fy,fz
  real*8,                                intent(in) :: X(ex(1)),Y(ex(2)),Z(ex(3))
