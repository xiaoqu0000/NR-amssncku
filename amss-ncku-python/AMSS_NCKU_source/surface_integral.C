...
// Emit a Perfetto trace span from a pre-captured start timestamp to now.
// Usage: SI_TRACE_EMIT("name", ts_start);
#define SI_TRACE_EMIT(name, ts_start) \
  sft_timer::add_kernel_trace((name), (ts_start), sft_timer::get_trace_timestamp(), myrank, 0)
#define SI_TS() sft_timer::get_trace_timestamp()
#else
#define SI_TRACE_EMIT(name, ts_start) (void)0
#define SI_TS() (uint64_t)0
#endif
...
{
  if (interp_shm) { delete interp_shm; interp_shm = nullptr; }
  delete[] nx_g;
  delete[] ny_g;
  delete[] nz_g;
  delete[] arcostheta;
...
{
  double t0 = MPI_Wtime();
  auto _sw_ts0 = SI_TS(); // trace: surf_wave_prepare start
  ...
  double t1 = MPI_Wtime();
  time_prepare += t1 - t0;
  auto _sw_ts1 = SI_TS(); SI_TRACE_EMIT("surf_wave_prepare", _sw_ts0);

  GH->PatL[lev]->data->Interp_Points(DG_List, n_tot, pox, shellf, Symmetry);

  double t2 = MPI_Wtime();
  time_interp += t2 - t1;
  auto _sw_ts2 = SI_TS(); SI_TRACE_EMIT("surf_wave_interp", _sw_ts1);
...
#pragma omp parallel
  {
    // per-thread local accumulation arrays to avoid race conditions
    double *RP_local = new double[NN]();
    double *IP_local = new double[NN]();

#pragma omp for private(n) schedule(static)
    for (n = Nmin; n <= Nmax; n++)
    {
      //       need round off always
      int i = int(n / N_phi); // int(1.723) = 1, int(-1.732) = -1
      int j = n - i * N_phi;
      ...
      for (int pl = spinw; pl < maxl + 1; pl++)
        for (int pm = -pl; pm < pl + 1; pm++)
        {
          for (int lp = 0; lp < lpsy; lp++)
          {
            ...
            RP_local[countlm] += thetap * (psi4RR * cosmphi + psi4II * sinmphi) * wtcostheta[i];
            IP_local[countlm] += thetap * (psi4II * cosmphi - psi4RR * sinmphi) * wtcostheta[i];
#else
            RP_local[countlm] += thetap * (psi4RR * cosmphi + psi4II * sinmphi);
            IP_local[countlm] += thetap * (psi4II * cosmphi - psi4RR * sinmphi);
#endif
          }
    ...
    // reduce thread-local results into shared output arrays
#pragma omp critical
    {
      for (int ii = 0; ii < NN; ii++)
      {
        RP_out[ii] += RP_local[ii];
        IP_out[ii] += IP_local[ii];
      }
    }
    delete[] RP_local;
    delete[] IP_local;
  ...
  double t3 = MPI_Wtime();
  time_integration += t3 - t2;
  auto _sw_ts3 = SI_TS(); SI_TRACE_EMIT("surf_wave_integration", _sw_ts2);

  MPI_Allreduce(RP_out, RP, NN, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
  MPI_Allreduce(IP_out, IP, NN, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);

  double t4 = MPI_Wtime();
  time_communication += t4 - t3;
  SI_TRACE_EMIT("surf_wave_comm", _sw_ts3);
  ...
  time_surf_wave += MPI_Wtime() - t0;
}

void surface_integral::surf_Wave_Batch(double *Rex_array, int num_rex, int lev, cgh *GH,
                                       var *Rpsi4, var *Ipsi4,
                                       ...
                                       monitor *Monitor)
{
  ...
  const int InList = 2;
  const int total_pts = num_rex * n_tot;

  MyList<var> *DG_List = new MyList<var>(Rpsi4);
  DG_List->insert(Ipsi4);

  // Build batched pox: all radii concatenated
  double *pox[3];
  for (int d = 0; d < 3; d++)
    pox[d] = new double[total_pts];
  for (int r = 0; r < num_rex; r++)
  {
    int offset = r * n_tot;
    for (int n = 0; n < n_tot; n++)
    {
      pox[0][offset + n] = Rex_array[r] * nx_g[n];
      pox[1][offset + n] = Rex_array[r] * ny_g[n];
      pox[2][offset + n] = Rex_array[r] * nz_g[n];
    }
  }

  double *shellf = new double[total_pts * InList];

  double t1 = MPI_Wtime();
  time_prepare += t1 - t0;
  auto _swb_ts1 = SI_TS(); SI_TRACE_EMIT("surf_wave_batch_prepare", _swb_ts0);

  // Single batched Interp_Points call
  if (interp_shm) {
    GH->PatL[lev]->data->Interp_Points_SHM(DG_List, total_pts, pox, shellf, Symmetry, interp_shm);
  } else {
    GH->PatL[lev]->data->Interp_Points(DG_List, total_pts, pox, shellf, Symmetry);
  }

  double t2 = MPI_Wtime();
  time_interp += t2 - t1;
  auto _swb_ts2 = SI_TS(); SI_TRACE_EMIT("surf_wave_batch_interp", _swb_ts1);

  // Integration: per-radius, distributed across ranks
  int mp, Lp, Nmin, Nmax;
  mp = n_tot / cpusize;
  Lp = n_tot - cpusize * mp;
  if (Lp > myrank)
  ...
  double *RP_out = new double[num_rex * NN]();
  double *IP_out = new double[num_rex * NN]();

  for (int r = 0; r < num_rex; r++)
  {
    double *shellf_r = shellf + r * n_tot * InList;
    double *RP_r = RP_out + r * NN;
    double *IP_r = IP_out + r * NN;
          ...
          for (int pm = -pl; pm < pl + 1; pm++)
          {
            for (int lp = 0; lp < lpsy; lp++)
            {
              ...
              case 0:
                ...
                cosmphi = cos(pm * (j + 0.5) * dphi);
                sinmphi = sin(pm * (j + 0.5) * dphi);
                psi4RR = shellf_r[InList * n];
                psi4II = shellf_r[InList * n + 1];
                break;
              case 1:
                costheta = -arcostheta[i];
                cosmphi = cos(pm * (j + 0.5) * dphi);
                sinmphi = sin(pm * (j + 0.5) * dphi);
                psi4RR = Rpsi4->SoA[2] * shellf_r[InList * n];
                psi4II = Ipsi4->SoA[2] * shellf_r[InList * n + 1];
                break;
              case 2:
                costheta = arcostheta[i];
                cosmphi = cos(pm * (j + 0.5) * dphi);
                sinmphi = -sin(pm * (j + 0.5) * dphi);
                psi4RR = Rpsi4->SoA[1] * shellf_r[InList * n];
                psi4II = Ipsi4->SoA[1] * shellf_r[InList * n + 1];
                break;
              case 3:
                costheta = -arcostheta[i];
                cosmphi = cos(pm * (j + 0.5) * dphi);
                sinmphi = -sin(pm * (j + 0.5) * dphi);
                psi4RR = Rpsi4->SoA[2] * Rpsi4->SoA[1] * shellf_r[InList * n];
                psi4II = Ipsi4->SoA[2] * Ipsi4->SoA[1] * shellf_r[InList * n + 1];
                break;
              case 4:
                costheta = arcostheta[i];
                cosmphi = cos(pm * (PI - (j + 0.5) * dphi));
                sinmphi = sin(pm * (PI - (j + 0.5) * dphi));
                psi4RR = Rpsi4->SoA[0] * shellf_r[InList * n];
                psi4II = Ipsi4->SoA[0] * shellf_r[InList * n + 1];
                break;
              case 5:
                costheta = -arcostheta[i];
                cosmphi = cos(pm * (PI - (j + 0.5) * dphi));
                sinmphi = sin(pm * (PI - (j + 0.5) * dphi));
                psi4RR = Rpsi4->SoA[2] * Rpsi4->SoA[0] * shellf_r[InList * n];
                psi4II = Ipsi4->SoA[2] * Ipsi4->SoA[0] * shellf_r[InList * n + 1];
                break;
              case 6:
                costheta = arcostheta[i];
                cosmphi = cos(pm * (PI + (j + 0.5) * dphi));
                sinmphi = sin(pm * (PI + (j + 0.5) * dphi));
                psi4RR = Rpsi4->SoA[1] * Rpsi4->SoA[0] * shellf_r[InList * n];
                psi4II = Ipsi4->SoA[1] * Ipsi4->SoA[0] * shellf_r[InList * n + 1];
                break;
              case 7:
                costheta = -arcostheta[i];
                cosmphi = cos(pm * (PI + (j + 0.5) * dphi));
                sinmphi = sin(pm * (PI + (j + 0.5) * dphi));
                psi4RR = Rpsi4->SoA[2] * Rpsi4->SoA[1] * Rpsi4->SoA[0] * shellf_r[InList * n];
                psi4II = Ipsi4->SoA[2] * Ipsi4->SoA[1] * Ipsi4->SoA[0] * shellf_r[InList * n + 1];
              }

              thetap = sqrt((2 * pl + 1.0) / 4.0 / PI) * misc::Wigner_d_function(pl, pm, spinw, costheta);
#ifdef GaussInt
              RP_local[countlm] += thetap * (psi4RR * cosmphi + psi4II * sinmphi) * wtcostheta[i];
              IP_local[countlm] += thetap * (psi4II * cosmphi - psi4RR * sinmphi) * wtcostheta[i];
#else
...
#pragma omp critical
    ...
    {
#ifdef GaussInt
      RP_r[ii] = RP_r[ii] * Rex_array[r] * dphi;
      IP_r[ii] = IP_r[ii] * Rex_array[r] * dphi;
#else
      RP_r[ii] = RP_r[ii] * Rex_array[r] * dphi * dcostheta;
      IP_r[ii] = IP_r[ii] * Rex_array[r] * dphi * dcostheta;
#endif
    }
  } // end radius loop
  ...
  double t3 = MPI_Wtime();
  time_integration += t3 - t2;
  auto _swb_ts3 = SI_TS(); SI_TRACE_EMIT("surf_wave_batch_integration", _swb_ts2);

  MPI_Allreduce(RP_out, RP_all, num_rex * NN, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
  MPI_Allreduce(IP_out, IP_all, num_rex * NN, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);

  double t4 = MPI_Wtime();
  time_communication += t4 - t3;
  SI_TRACE_EMIT("surf_wave_batch_comm", _swb_ts3);
...
{
  double t0 = MPI_Wtime();
  auto _smp_ts0 = SI_TS(); // trace: surf_masspang_prepare start
  if (myrank == 0 && GH->grids[lev] != 1)
  ...
  SI_TRACE_EMIT("surf_masspang_f_admmass", _smp_ts0);
  ...
  double t1 = MPI_Wtime();
  time_prepare += t1 - t0;
  auto _smp_ts1 = SI_TS(); SI_TRACE_EMIT("surf_masspang_prepare", _smp_ts0);
  ...
  double t2 = MPI_Wtime();
  time_interp += t2 - t1;
  auto _smp_ts2 = SI_TS(); SI_TRACE_EMIT("surf_masspang_interp", _smp_ts1);
  ...
  int i;
#pragma omp parallel for reduction(+:Mass_out,ang_outx,ang_outy,ang_outz,p_outx,p_outy,p_outz) \
                         private(Chi,Psi,Gxx,Gxy,Gxz,Gyy,Gyz,Gzz,gupxx,gupxy,gupxz,gupyy,gupyz,gupzz,TRK,axx,axy,axz,ayy,ayz,azz,aupxx,aupxy,aupxz,aupyx,aupyy,aupyz,aupzx,aupzy,aupzz,i)
  for (n = Nmin; n <= Nmax; n++)
  {
    //       need round off always
    i = int(n / N_phi); // int(1.723) = 1, int(-1.732) = -1
...
// but this factor has been considered in f_admmass_bssn
  ...
  double t3 = MPI_Wtime();
  time_integration += t3 - t2;
  auto _smp_ts3 = SI_TS(); SI_TRACE_EMIT("surf_masspang_integration", _smp_ts2);
  ...
  double t4 = MPI_Wtime();
  time_communication += t4 - t3;
  SI_TRACE_EMIT("surf_masspang_comm", _smp_ts3);
  ...
  delete[] shellf;
  DG_List->clearList();
  time_surf_masspang += MPI_Wtime() - t0;
}
void surface_integral::surf_MassPAng_Batch(double *Rex_array, int num_rex, int lev, cgh *GH,
                                           var *chi, var *trK,
                                           var *gxx, var *gxy, var *gxz, var *gyy, var *gyz, var *gzz,
                                           var *Axx, var *Axy, var *Axz, var *Ayy, var *Ayz, var *Azz,
                                           var *Gmx, var *Gmy, var *Gmz,
                                           ...
                                           double *Rout_all, monitor *Monitor)
{
  ...
  // f_admmass_bssn: radius-independent, call once
  MyList<Patch> *Pp = GH->PatL[lev];
  while (Pp)
  {
    MyList<Block> *BP = Pp->data->blb;
  ...
  SI_TRACE_EMIT("surf_masspang_batch_f_admmass", _smpb_ts0);
  ...
  double t1 = MPI_Wtime();
  time_prepare += t1 - t0;
  auto _smpb_ts1 = SI_TS(); SI_TRACE_EMIT("surf_masspang_batch_prepare", _smpb_ts0);
  ...
  double t2 = MPI_Wtime();
  time_interp += t2 - t1;
  auto _smpb_ts2 = SI_TS(); SI_TRACE_EMIT("surf_masspang_batch_interp", _smpb_ts1);
  ...
  double *reduce_out = new double[num_rex * 7]();
  double *reduce_in  = new double[num_rex * 7]();

  for (int r = 0; r < num_rex; r++)
  {
    double *shellf_r = shellf + r * n_tot * InList;
    int pox_offset = r * n_tot;

    double Mass_out = 0;
    double ang_outx = 0, ang_outy = 0, ang_outz = 0;
    double p_outx = 0, p_outy = 0, p_outz = 0;
...
#pragma omp parallel for reduction(+:Mass_out,ang_outx,ang_outy,ang_outz,p_outx,p_outy,p_outz) \
    ...
    for (n = Nmin; n <= Nmax; n++)
    {
      ...
      Chi = shellf_r[InList * n + 3];
      TRK = shellf_r[InList * n + 4];
      Gxx = shellf_r[InList * n + 5] + 1.0;
      Gxy = shellf_r[InList * n + 6];
      Gxz = shellf_r[InList * n + 7];
      Gyy = shellf_r[InList * n + 8] + 1.0;
      Gyz = shellf_r[InList * n + 9];
      Gzz = shellf_r[InList * n + 10] + 1.0;
      axx = shellf_r[InList * n + 11];
      axy = shellf_r[InList * n + 12];
      axz = shellf_r[InList * n + 13];
      ayy = shellf_r[InList * n + 14];
      ayz = shellf_r[InList * n + 15];
      azz = shellf_r[InList * n + 16];

      Chi = 1.0 / (1.0 + Chi);
      Psi = Chi * sqrt(Chi);

#ifdef GaussInt
      Mass_out = Mass_out + (shellf_r[InList * n] * nx_g[n] + shellf_r[InList * n + 1] * ny_g[n] + shellf_r[InList * n + 2] * nz_g[n]) * wtcostheta[i];
#else
      Mass_out = Mass_out + (shellf_r[InList * n] * nx_g[n] + shellf_r[InList * n + 1] * ny_g[n] + shellf_r[InList * n + 2] * nz_g[n]);
#endif
      ...
      {
#ifdef GaussInt
        ang_outx = ang_outx + f1o8 * Psi * (nx_g[n] * (pox[1][pox_offset + n] * aupxz - pox[2][pox_offset + n] * aupxy) + ny_g[n] * (pox[1][pox_offset + n] * aupyz - pox[2][pox_offset + n] * aupyy) + nz_g[n] * (pox[1][pox_offset + n] * aupzz - pox[2][pox_offset + n] * aupzy)) * wtcostheta[i];
        ang_outy = ang_outy + f1o8 * Psi * (nx_g[n] * (pox[2][pox_offset + n] * aupxx - pox[0][pox_offset + n] * aupxz) + ny_g[n] * (pox[2][pox_offset + n] * aupyx - pox[0][pox_offset + n] * aupyz) + nz_g[n] * (pox[2][pox_offset + n] * aupzx - pox[0][pox_offset + n] * aupzz)) * wtcostheta[i];
        ang_outz = ang_outz + f1o8 * Psi * (nx_g[n] * (pox[0][pox_offset + n] * aupxy - pox[1][pox_offset + n] * aupxx) + ny_g[n] * (pox[0][pox_offset + n] * aupyy - pox[1][pox_offset + n] * aupyx) + nz_g[n] * (pox[0][pox_offset + n] * aupzy - pox[1][pox_offset + n] * aupzx)) * wtcostheta[i];
#else
        ang_outx = ang_outx + f1o8 * Psi * (nx_g[n] * (pox[1][pox_offset + n] * aupxz - pox[2][pox_offset + n] * aupxy) + ny_g[n] * (pox[1][pox_offset + n] * aupyz - pox[2][pox_offset + n] * aupyy) + nz_g[n] * (pox[1][pox_offset + n] * aupzz - pox[2][pox_offset + n] * aupzy));
        ang_outy = ang_outy + f1o8 * Psi * (nx_g[n] * (pox[2][pox_offset + n] * aupxx - pox[0][pox_offset + n] * aupxz) + ny_g[n] * (pox[2][pox_offset + n] * aupyx - pox[0][pox_offset + n] * aupyz) + nz_g[n] * (pox[2][pox_offset + n] * aupzx - pox[0][pox_offset + n] * aupzz));
        ang_outz = ang_outz + f1o8 * Psi * (nx_g[n] * (pox[0][pox_offset + n] * aupxy - pox[1][pox_offset + n] * aupxx) + ny_g[n] * (pox[0][pox_offset + n] * aupyy - pox[1][pox_offset + n] * aupyx) + nz_g[n] * (pox[0][pox_offset + n] * aupzy - pox[1][pox_offset + n] * aupzx));
#endif
      }
      else if (Symmetry == 1)
      {
    ...
    double rex = Rex_array[r];
#ifdef GaussInt
    reduce_out[r * 7 + 0] = Mass_out * rex * rex * dphi * factor;
    reduce_out[r * 7 + 1] = p_outx * rex * rex * dphi * (1.0 / PI) * factor;
    reduce_out[r * 7 + 2] = p_outy * rex * rex * dphi * (1.0 / PI) * factor;
    reduce_out[r * 7 + 3] = p_outz * rex * rex * dphi * (1.0 / PI) * factor;
    reduce_out[r * 7 + 4] = ang_outx * rex * rex * dphi * (1.0 / PI) * factor;
    reduce_out[r * 7 + 5] = ang_outy * rex * rex * dphi * (1.0 / PI) * factor;
    reduce_out[r * 7 + 6] = ang_outz * rex * rex * dphi * (1.0 / PI) * factor;
#else
    reduce_out[r * 7 + 0] = Mass_out * rex * rex * dphi * dcostheta * factor;
    reduce_out[r * 7 + 1] = p_outx * rex * rex * dphi * dcostheta * (1.0 / PI) * factor;
    reduce_out[r * 7 + 2] = p_outy * rex * rex * dphi * dcostheta * (1.0 / PI) * factor;
    reduce_out[r * 7 + 3] = p_outz * rex * rex * dphi * dcostheta * (1.0 / PI) * factor;
    reduce_out[r * 7 + 4] = ang_outx * rex * rex * dphi * dcostheta * (1.0 / PI) * factor;
    reduce_out[r * 7 + 5] = ang_outy * rex * rex * dphi * dcostheta * (1.0 / PI) * factor;
    reduce_out[r * 7 + 6] = ang_outz * rex * rex * dphi * dcostheta * (1.0 / PI) * factor;
#endif
  } // end radius loop

  double t3 = MPI_Wtime();
  time_integration += t3 - t2;
  auto _smpb_ts3 = SI_TS(); SI_TRACE_EMIT("surf_masspang_batch_integration", _smpb_ts2);

  MPI_Allreduce(reduce_out, reduce_in, num_rex * 7, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);

  for (int r = 0; r < num_rex; r++)
  {
    Rout_all[r * 7 + 0] = reduce_in[r * 7 + 0];
    Rout_all[r * 7 + 1] = reduce_in[r * 7 + 1];
    Rout_all[r * 7 + 2] = reduce_in[r * 7 + 2];
    Rout_all[r * 7 + 3] = reduce_in[r * 7 + 3];
    Rout_all[r * 7 + 4] = reduce_in[r * 7 + 4];
    Rout_all[r * 7 + 5] = reduce_in[r * 7 + 5];
    Rout_all[r * 7 + 6] = reduce_in[r * 7 + 6];
  }

  double t4 = MPI_Wtime();
  time_communication += t4 - t3;
  SI_TRACE_EMIT("surf_masspang_batch_comm", _smpb_ts3);
...
// but this factor has been considered in f_admmass_bssn
  ...
  double t_int_end = MPI_Wtime();
  time_integration += t_int_end - t_int_start;
  ...
  time_communication += t_comm_end - t_int_end;
