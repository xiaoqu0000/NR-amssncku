...
subroutine kodis_inner(ex,X,Y,Z,fh,f_rhs,Symmetry,eps)
...
! calculation order if important ?
  ...
  enddo
!$OMP END DO NOWAIT
  call sft_trace("kodis_inner", ts0_thr, sft_get_ts(), 0, tid)

  return

  end subroutine kodis_inner
...
double precision,intent(inout),dimension(ex(1),ex(2),ex(3))::f_rhs
real*8,intent(in) :: eps
write(0,*) 'kodis_inner: stub for ghost_width=4 invoked unexpectedly'
stop 1
end subroutine kodis_inner
