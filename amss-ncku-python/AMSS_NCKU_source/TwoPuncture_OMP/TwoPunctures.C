                                ...
                                double **JFD)
{
  ...
  if (n2 > MAX_N) {
    fprintf(stderr, "LineRelax_be: n2=%d exceeds MAX_N=%d (thread_local buffer overflow). "
                    "Raise MAX_N in TwoPunctures.C and recompile.\n", n2, MAX_N);
    abort();
  }
                                ...
                                int **cols, double **JFD)
{
  ...
  static const int MAX_N = 512;
  if (n1 > MAX_N) {
    fprintf(stderr, "LineRelax_al: n1=%d exceeds MAX_N=%d (thread_local buffer overflow). "
                    "Raise MAX_N in TwoPunctures.C and recompile.\n", n1, MAX_N);
    abort();
  }
...
void TwoPunctures::ThomasAlgorithm(int N, double *b, double *a, double *c, double *x, double *q)
{
  ...
  static const int MAX_N = 512;
  if (N > MAX_N) {
    fprintf(stderr, "ThomasAlgorithm: N=%d exceeds MAX_N=%d (thread_local buffer overflow). "
                    "Raise MAX_N in TwoPunctures.C and recompile.\n", N, MAX_N);
    abort();
  }
