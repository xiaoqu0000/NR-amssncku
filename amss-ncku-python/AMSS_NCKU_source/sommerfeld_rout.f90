...
!!!$OMP PARALLEL PRIVATE(i,j,k,ts0_thr,tid) SHARED(f,f0,layer,gp)
...
!!!$OMP DO COLLAPSE(1)
     ...
     enddo
!!!$OMP END DO
  call sft_trace("sommerfeld_rout", ts0_thr, sft_get_ts(), 0, tid)
!!!$OMP END PARALLEL
    endif
   enddo
...
!!!$OMP PARALLEL PRIVATE(i,j,k,r,fac,cx,cxB,cxT,ya,ddy,nouse,m,ts0_thr,tid) SHARED(f,f0,chi0,Lap0,X,Y,Z,dX,dY,dZ,dT,ex,SoA,xa,layer,gp,Symmetry)
  tid = omp_get_thread_num()
  ts0_thr = sft_get_ts()
!!!$OMP DO COLLAPSE(1)
...
!!!$OMP PARALLEL PRIVATE(i,j,k,fx,fy,fz,R,wx,wy,wz,ts0_thr,tid) DEFAULT(SHARED)
  tid = omp_get_thread_num()
  ts0_thr = sft_get_ts()
!!!$OMP DO COLLAPSE(1)
    do k = layer(3,gp), layer(6,gp)
...
!! we always assume var0 = 0
    ...
    enddo
!!!$OMP END DO
    call sft_trace("sommerfeld_routbam", ts0_thr, sft_get_ts(), 0, tid)
!!!$OMP END PARALLEL
   endif
  enddo
