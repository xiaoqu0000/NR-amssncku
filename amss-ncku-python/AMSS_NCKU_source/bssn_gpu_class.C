...
// This member function configures the single-step time evolution for each grid level during the time evolution process.
// For the case PSTR == 0
...
// 该成员函数设定了时间演化过程中的每层网格的单步时间演化（针对 PSTR == 1 的情形）
...
// This member function sets up the single-step time evolution for the spherical shell grid part during the time evolution process
