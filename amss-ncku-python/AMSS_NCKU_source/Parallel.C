                          ...
                          MyList<var> *VarLists /* source */, MyList<var> *VarListd /* target */, int Symmetry)
{
  ...
  // Pre-count variables and build arrays from linked lists
  int nvar = 0;
  {
    MyList<var> *tmp = VarLists;
    while (tmp) { nvar++; tmp = tmp->next; }
  }
  {
    MyList<var> *tmp = VarListd;
    int nvar_d = 0;
    while (tmp) { nvar_d++; tmp = tmp->next; }
    if (nvar != nvar_d) {
      cout << "error in short data packer, var lists does not match." << endl;
      MPI_Abort(MPI_COMM_WORLD, 1);
    }
  }
  int *sgfn_s = new int[nvar];
  int *sgfn_d = new int[nvar];
  double *SoA_arr = new double[nvar * 3];
  {
    MyList<var> *vs = VarLists, *vd = VarListd;
    for (int v = 0; v < nvar; v++) {
      sgfn_s[v] = vs->data->sgfn;
      sgfn_d[v] = vd->data->sgfn;
      for (int d = 0; d < 3; d++) SoA_arr[v * 3 + d] = vs->data->SoA[d];
      vs = vs->next;
      vd = vd->next;
    }
  }
  ...
  while (src && dst)
  {
    ...
    {
      int seg_size = dst->data->shape[0] * dst->data->shape[1] * dst->data->shape[2];
      if (data)
      {
        if (dir == PACK)
        {
          ...
          for (int v = 0; v < nvar; v++)
          {
            int offset = size_out + v * seg_size;
            switch (type)
            {
            ...
            case 1:
              f_copy(DIM, dst->data->llb, dst->data->uub, dst->data->shape, data + offset,
                     src->data->Bg->bbox, src->data->Bg->bbox + dim, src->data->Bg->shape, src->data->Bg->fgfs[sgfn_s[v]],
                     dst->data->llb, dst->data->uub);
              break;
            case 2:
              f_restrict3(DIM, dst->data->llb, dst->data->uub, dst->data->shape, data + offset,
                          src->data->Bg->bbox, src->data->Bg->bbox + dim, src->data->Bg->shape, src->data->Bg->fgfs[sgfn_s[v]],
                          dst->data->llb, dst->data->uub, SoA_arr + v * 3, Symmetry);
              break;
            case 3:
              f_prolong3(DIM, src->data->Bg->bbox, src->data->Bg->bbox + dim, src->data->Bg->shape, src->data->Bg->fgfs[sgfn_s[v]],
                         dst->data->llb, dst->data->uub, dst->data->shape, data + offset,
                         dst->data->llb, dst->data->uub, SoA_arr + v * 3, Symmetry);
              break;
            }
          }
        ...
        if (dir == UNPACK)
        {
          ...
          {
            int offset = size_out + v * seg_size;
            f_copy(DIM, dst->data->Bg->bbox, dst->data->Bg->bbox + dim, dst->data->Bg->shape, dst->data->Bg->fgfs[sgfn_d[v]],
                   dst->data->llb, dst->data->uub, dst->data->shape, data + offset,
                   dst->data->llb, dst->data->uub);
          }
        }
      }
      size_out += nvar * seg_size;
    }
    dst = dst->next;
    src = src->next;
  }
                             ...
                             MyList<var> *VarLists /* source */, MyList<var> *VarListd /* target */, int Symmetry)
{
  ...
  while (src && dst)
  {
        ...
        (dir == UNPACK && src->data->Bg->rank == rank_in && dst->data->Bg->rank == myrank))
    {
      ...
      int seg_size = (src->data->shape[0] + 2 * ghost_width) * (src->data->shape[1] + 2 * ghost_width) * (src->data->shape[2] + 2 * ghost_width);
      if (data)
      {
        if (dir == PACK)
        {
          ...
          {
            int offset = size_out + v * seg_size;
            f_prolongcopy3(DIM, src->data->Bg->bbox, src->data->Bg->bbox + dim, src->data->Bg->shape, src->data->Bg->fgfs[sgfn_s[v]],
                           dst->data->llb, dst->data->uub, src->data->shape, data + offset,
                           src->data->llb, src->data->uub, SoA_arr + v * 3, Symmetry);
          }
        }
        if (dir == UNPACK)
        {
          ...
          {
            int offset = size_out + v * seg_size;
            f_prolongmix3(DIM, dst->data->Bg->bbox, dst->data->Bg->bbox + dim, dst->data->Bg->shape, dst->data->Bg->fgfs[sgfn_d[v]],
                          src->data->llb, src->data->uub, src->data->shape, data + offset,
                          dst->data->llb, dst->data->uub, SoA_arr + v * 3, Symmetry, dst->data->illb, dst->data->iuub);
          }
        }
      }
      size_out += nvar * seg_size;
