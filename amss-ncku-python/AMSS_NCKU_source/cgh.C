...
// 该成员函数用于构建网格（针对 PSTR == 1 和 PSTR == 2 的情形）
// This member function constructs the computational grid
...
// 该成员函数用于将网格移动
// This member function shifts the grid
...
MyList<Patch> *cgh::construct_patchlist(int lev, int Symmetry)
{
  ...
  */
  ...
  // Merge grid boxes (merging more than three grid boxes may cause bugs)
  ...
  // When grid boxes overlap, re-split the grid boxes
...
// 该成员函数用于生对单层网格进行重构（针对 PSTR == 0 的情形？）
