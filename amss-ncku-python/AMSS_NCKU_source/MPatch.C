...
#define IP_TRACE_EMIT(name, ts_start) \
  sft_timer::add_kernel_trace((name), (ts_start), sft_timer::get_trace_timestamp(), myrank, 0)
#define IP_TRACE_EMIT_TID(name, ts_start, tid) \
  sft_timer::add_kernel_trace((name), (ts_start), sft_timer::get_trace_timestamp(), myrank, (tid))
#define IP_TS() sft_timer::get_trace_timestamp()
#else
#define IP_TRACE_EMIT(name, ts_start) (void)0
#define IP_TRACE_EMIT_TID(name, ts_start, tid) (void)0
#define IP_TS() (uint64_t)0
#endif
                          ...
                          double *Shellf, int Symmetry)
{
  ...
  int myrank;
  MPI_Comm_rank(MPI_COMM_WORLD, &myrank);
  auto _ip_ts0 = IP_TS(); // trace: interp_points_omp start
...
#pragma omp parallel
  {
    auto _omp_ts0 = IP_TS(); // per-thread start for interp_points_omp
#pragma omp for schedule(dynamic)
    for (int j = 0; j < NN; j++) // run along points
    {
      double pox[dim];
      double llb[dim], uub[dim]; // thread-local shadow to avoid race condition
      MyList<var> *varl;         // thread-local shadow to avoid race condition
      for (int i = 0; i < dim; i++)
      {
        pox[i] = XX[i][j];
...
#error Not define Vertex nor Cell
    ...
    IP_TRACE_EMIT_TID("interp_points_omp", _omp_ts0, omp_get_thread_num());
  } // end omp parallel
  auto _ip_ts1 = IP_TS(); IP_TRACE_EMIT("interp_points_omp_wall", _ip_ts0);
  ...
  IP_TRACE_EMIT("interp_points_comm", _ip_ts1);
...
void Patch::Interp_Points_SHM(MyList<var> *VarList,
                               int NN, double **XX,
                               double *Shellf, int Symmetry,
                               InterpSHM *shm)
{
    ...
    // Count variables and collect sgfn indices + SoA arrays
    MyList<var> *varl = VarList;
    int num_var = 0;
    while (varl) { num_var++; varl = varl->next; }

    int *var_sgfns = new int[num_var];
    double *var_SoA = new double[num_var * 3]; // flattened [num_var][3]
    varl = VarList;
    for (int k = 0; k < num_var; k++) {
        var_sgfns[k] = varl->data->sgfn;
        for (int d = 0; d < 3; d++) var_SoA[k * 3 + d] = varl->data->SoA[d];
        varl = varl->next;
    }

    double DH[dim];
    for (int i = 0; i < dim; i++) DH[i] = getdX(i);

    int ordn = 2 * ghost_width;

    // Phase 1: Gather block data into shared memory
    int num_blocks = shm->gather(blb, ble, var_sgfns, num_var, XX, NN);
    auto _ip_ts1 = IP_TS();
    IP_TRACE_EMIT("interp_points_shm_gather", _ip_ts0);

    // Phase 2: Distributed interpolation — each rank takes its slice
    int my_start = (NN * shm->numa_rank) / shm->numa_size;
    int my_end   = (NN * (shm->numa_rank + 1)) / shm->numa_size;

    // Build a map from global block index → local block_table index.
    // We must walk the global block list (blb..ble) in the SAME order as
    // the non-SHM Interp_Points so that all NUMA groups agree on which
    // block is "canonical" (= first match in global order) for each
    // boundary point. Otherwise different NUMA groups may each claim the
    // same boundary point from their own local block_table, producing
    // Weight[i] > 1 in the cross-NUMA Allreduce.
    std::unordered_map<int, int> gbi_to_local;
    gbi_to_local.reserve((size_t)num_blocks * 2);
    for (int bi = 0; bi < num_blocks; bi++) {
        gbi_to_local.emplace(shm->block_table[bi].block_id, bi);
    }
...
#pragma omp for schedule(dynamic)
        ...
        {
            double pox[dim];
            double llb_local[dim], uub_local[dim];
            for (int i = 0; i < dim; i++)
                pox[i] = XX[i][j];

            // Walk the global block list in the canonical order. The first
            // block whose extended bbox contains pox is the canonical block.
            // Only the NUMA group that owns that block performs the
            // interpolation and sets weight=1; other groups break out
            // silently. This mirrors Interp_Points (non-SHM) which already
            // applies "first-match-wins + owner-only-weight" semantics.
            MyList<Block> *Bp = blb;
            int gbi = 0;
            bool notfind = true;
            while (notfind && Bp) {
...
#error Both Cell and Vertex are defined
#endif
                    llb_local[i] = (feq(BP->bbox[i], bbox[i], DH[i] / 2))
                        ? BP->bbox[i] + lli[i] * DH[i]
                        : BP->bbox[i] + (ghost_width - 0.5) * DH[i];
                    uub_local[i] = (feq(BP->bbox[dim + i], bbox[dim + i], DH[i] / 2))
...
#error Not define Vertex nor Cell
                ...
                if (flag) {
                    notfind = false;
                    auto it = gbi_to_local.find(gbi);
                    if (it != gbi_to_local.end()) {
                        // This NUMA group owns the canonical block.
                        const ShmBlockInfo &info = shm->block_table[it->second];
                        char *base = (char *)shm->fgfs_shm;
                        double *X_shm[dim];
                        for (int d = 0; d < dim; d++)
                            X_shm[d] = (double *)(base + info.X_offset[d]);

                        for (int k = 0; k < num_var; k++) {
                            double *fgfs_ptr = (double *)(base + info.fgfs_offset
                                + (size_t)k * info.nn * sizeof(double));
                            f_global_interp(const_cast<int*>(info.shape),
                                            X_shm[0], X_shm[1], X_shm[2],
                                            fgfs_ptr,
                                            shm->shellf_shm[j * num_var + k],
                                            pox[0], pox[1], pox[2],
                                            ordn, &var_SoA[k * 3], Symmetry);
                        }
                        shm->weight_shm[j] = 1;
                    }
                    // else: another NUMA group owns this block; leave weight=0
                }
                if (Bp == ble) break;
                Bp = Bp->next;
                gbi++;
        ...
        IP_TRACE_EMIT_TID("interp_points_shm_omp", _omp_ts0, omp_get_thread_num());
    } // end omp parallel

    auto _ip_ts2 = IP_TS();
    IP_TRACE_EMIT("interp_points_shm_compute", _ip_ts1);

    // Phase 3: Cross-NUMA reduce
    int *Weight = new int[NN];
    shm->reduce(Shellf, Weight, NN, num_var);

    auto _ip_ts3 = IP_TS();
    IP_TRACE_EMIT("interp_points_shm_reduce", _ip_ts2);

    // Phase 4: Weight check (same as original)
    for (int i = 0; i < NN; i++) {
        if (Weight[i] > 1) {
            if (myrank == 0)
                cout << "WARNING: Patch::Interp_Points_SHM meets multiple weight" << endl;
            for (int j = 0; j < num_var; j++)
                Shellf[j + i * num_var] /= Weight[i];
        } else if (Weight[i] == 0 && myrank == 0) {
            cout << "ERROR: Patch::Interp_Points_SHM point ("
