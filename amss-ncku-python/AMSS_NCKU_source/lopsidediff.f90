...
subroutine lopsided_inner(ex,X,Y,Z,fh,f_rhs,Sfx,Sfy,Sfz,Symmetry)
  use sft_trace_mod
  use omp_lib
  implicit none
  integer(8) :: ts0_thr
...
! set kmax and kmin 0
  ...
  enddo
!$OMP END DO NOWAIT
  call sft_trace("lopsided_inner", ts0_thr, sft_get_ts(), 0, tid)

  return

  end subroutine lopsided_inner
...
! Stub for ghost_width=4: needed for linker; compute_rhs_bssn_opt path is
! never executed when Equation_Class != "BSSN".
subroutine lopsided_inner(ex,X,Y,Z,fh,f_rhs,Sfx,Sfy,Sfz,Symmetry)
  implicit none
  integer, intent(in)  :: ex(1:3),Symmetry
  real*8,  intent(in)  :: X(1:ex(1)),Y(1:ex(2)),Z(1:ex(3))
  ...
  real*8,dimension(ex(1),ex(2),ex(3)),intent(inout):: f_rhs
  real*8,dimension(-2:ex(1),-2:ex(2),-2:ex(3)),intent(in) :: fh
  write(0,*) 'lopsided_inner: stub for ghost_width=4 invoked unexpectedly'
  stop 1
end subroutine lopsided_inner
