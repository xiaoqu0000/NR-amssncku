...
#include <cstdio>
#include <cstdlib>
#include <cstring>    // memcpy (Fortran wrapper)
#include <fstream>
#include <iomanip>
#include <mutex>
#include <numeric>
...
static void init_trace() {
  std::call_once(g_trace_init_flag, []() {
    ...
    // Build default path with PID so MPI ranks don't collide.
    // Override with SFT_TRACE_PATH env var (e.g. "rank_0_trace.json") if needed.
    const char* env_path = std::getenv("SFT_TRACE_PATH");
    if (env_path && env_path[0] != '\0') {
      g_trace_output_path = env_path;
    } else {
      char default_path[64];
      snprintf(default_path, sizeof(default_path), "sft_trace_%d.json", (int)getpid());
      g_trace_output_path = default_path;
    }
    // Register signal handlers NOW so Ctrl+C (SIGINT from mpirun) also flushes.
    // Must be done before any signal can arrive.
...
void print_op_stats(InNumaPool* pool, const char* op_name) {
  ...
  // Signal handlers are now registered early in init_trace(); no need to re-register here.
...
// Public flush: call before MPI_Finalize() to guarantee the file is written
// even if atexit were not to run (e.g. unusual shutdown paths).
void flush() { write_trace_to_file(); }
...
// =====================================================
// Fortran ISO_C_BINDING interop wrappers
// Allow Fortran OMP threads to emit kernel trace events.
// Usage in Fortran: see sft_trace_mod.f90
// =====================================================
extern "C" {

// Returns current RDTSC timestamp as uint64 (opaque to Fortran, use integer(8)).
uint64_t sft_c_get_ts() {
#ifndef SFT_TIMER_DISABLED
  return sft_timer::get_trace_timestamp();
#else
  return 0;
#endif
}

// Add a kernel trace event from any thread (C or Fortran).
// name_len: length of the name string (Fortran passes character array without null).
void sft_c_add_trace(const char* name, int name_len, uint64_t ts0, uint64_t ts1, int pid, int tid) {
#ifndef SFT_TIMER_DISABLED
  // Copy name to a null-terminated buffer (Fortran strings are not null-terminated)
  char buf[128];
  int len = (name_len < 127) ? name_len : 127;
  // Trim trailing spaces (Fortran pads with spaces)
  while (len > 0 && name[len - 1] == ' ') len--;
  memcpy(buf, name, len);
  buf[len] = '\0';
  sft_timer::add_kernel_trace(buf, ts0, ts1, pid, tid);
#endif
}
