...
!$OMP PARALLEL PRIVATE(i,j,k,ts0_thr,tid) SHARED(f1,f0,f_rhs,ex) &
...
!$OMP DO COLLAPSE(1)
   do k=1,ex(3)
    do j=1,ex(2)
     do i=1,ex(1)
      f1(i,j,k) = f0(i,j,k) + dT * f_rhs(i,j,k)
     end do
    end do
   end do
!$OMP END DO
  call sft_trace("icn_rout", ts0_thr, sft_get_ts(), 0, tid)
!$OMP END PARALLEL
...
!$OMP DO COLLAPSE(1)
   do k=1,ex(3)
    do j=1,ex(2)
     do i=1,ex(1)
      f1(i,j,k) = f0(i,j,k) + HLF * dT * (f1(i,j,k) + f_rhs(i,j,k))
     end do
    end do
   end do
!$OMP END DO
...
!$OMP END DO
  call sft_trace("euler_rout", ts0_thr, sft_get_ts(), 0, tid)
!$OMP END PARALLEL
