...
File_directory   = "Automatically_BBH_GW150914"                    ## 程序运行目录             output file directory
...
Dissipation              = 0.2                    ## 耗散因子                                                                                    Kreiss-Oliger Dissipation Strength
...
shell_grid_number  = [48, 48, 400]                   ## 设置最外层球状网格（shell patch）的格点数目     grid points of Shell-Patch grid
...
quarter_sphere_number = 96                           ## 1/4 球面积分的格点数目
                                                     ## grid number of 1/4 s pherical surface
...
## parameter_BH[2] = [ 1.0/8.0,  0.0,  0.0 ]   ## 多黑动手动添加 
...
## 这一步的与初值求解中的 Ansorg-TwoPuncture 配合使用中需要注意的问题
...
position_BH[0]  = [ +0.0, +5.0*29.0/(36.0+29.0), 0.0 ]  
position_BH[1]  = [ -0.0, -5.0*36.0/(36.0+29.0), 0.0 ] 
## position_BH[2]  = [ -3.0,+3.0,+3.0 ]  ## 多黑洞手动添加
