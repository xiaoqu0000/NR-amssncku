...
inline void sub_fdderivs(double * f,double *fh,double *fxx,double *fxy,double *fxz,double *fyy,double *fyz,double *fzz,double* SoA)
{
	...
	// warning #20091-D: a __constant__ variable "_3D_SIZE" cannot be directly read in a host function
	...
	int hostSize[3];
        cudaMemcpyToSymbol("_3D_SIZE", hostSize, sizeof(int) * 3);  // 读取设备常量变量
	cudaMemset(fxx, 0, hostSize[0] * sizeof(double));
	cudaMemset(fxy, 0, hostSize[0] * sizeof(double));
	cudaMemset(fxz, 0, hostSize[0] * sizeof(double));
	cudaMemset(fyy, 0, hostSize[0] * sizeof(double));
	cudaMemset(fyz, 0, hostSize[0] * sizeof(double));
	cudaMemset(fzz, 0, hostSize[0] * sizeof(double));
	cudaDeviceSynchronize(); 
...
inline void sub_fderivs(double * f,double * fh,double *fx,double *fy,double *fz,double * SoA)
{
	...
	int hostSize[3];
        cudaMemcpyToSymbol("_3D_SIZE", hostSize, sizeof(int) * 3);  // 读取设备常量变量
        cudaMemset(fx, 0, hostSize[0] * sizeof(double));
	cudaMemset(fy, 0, hostSize[0] * sizeof(double));
	cudaMemset(fz, 0, hostSize[0] * sizeof(double));
