//$Id: Ansorg.C,v 1.1.1.1 2017/09/25 02:35:55 zjcao Exp $
#ifdef newc
#include <iostream>
#include <iomanip>
#include <fstream>
 ...
 Ansorg::Ansorg(char *filename,int orderi):pu_ps(0),coordA(0),coordB(0),coordphi(0)
{
  ...
  while (fgets(s, 1000, fp)) {
   ...
   assert(dz == 0);
    ...
    /* z-axis, note we assume the system is axisymmetric along z-axis */
    else if (dz != 0 && x1 == 0 && x2 == 0 && y1 == 0 && y2 == 0) {
      ps_b = dz/2;
      ps_dx = (z1+z2)/2;
      ps_rxx = 1;
      ps_rxy = 0;
...
//|----------------------------------------------------------------------------
void Ansorg::write_Pablo_file(const int *ext,const double xmin,const double xmax,const double ymin,const double ymax,const double zmin,const double zmax,
		     const char *filename)
{
...
//|--->open out put file  
    ...
    if(!outfile)
    {
      cout << "Ansorg::write_Pablo_file can't open " << filename << " for output." << endl;
      exit(0);
    }
