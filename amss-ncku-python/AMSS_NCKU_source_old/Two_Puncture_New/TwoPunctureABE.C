// $Id: TwoPunctureABE.C,v 1.1.1.1 2017/09/25 02:35:55 zjcao Exp $
#ifdef newc
#include <algorithm>   
#include <functional> 
#include <vector>
...
// read parameter from file
	...
	if(sgrp == "ABE")
	{
         ...
         else if ( skey == "P_plusx" )   P_plusx = atof(sval.c_str());
         else if ( skey == "P_plusy" )   P_plusy = atof(sval.c_str());
         else if ( skey == "P_plusz" )   P_plusz = atof(sval.c_str());
         else if ( skey == "P_minusx" )   P_minusx = atof(sval.c_str());
         else if ( skey == "P_minusy" )   P_minusy = atof(sval.c_str());
         else if ( skey == "P_minusz" )   P_minusz = atof(sval.c_str());
         else if ( skey == "S_plusx" )   S_plusx = atof(sval.c_str());
         else if ( skey == "S_plusy" )   S_plusy = atof(sval.c_str());
         else if ( skey == "S_plusz" )   S_plusz = atof(sval.c_str());
         else if ( skey == "S_minusx" )   S_minusx = atof(sval.c_str());
         else if ( skey == "S_minusy" )   S_minusy = atof(sval.c_str());
...
// echo parameters    
    {
      cout<<"///////////////////////////////////////////////////////////////"<<endl;
      cout<<"We are using coordinate which lets BBHs sit on x-axis:"<<endl;
      cout<<"     mp     = "<<mp<<endl;
      cout<<"     mm     = "<<mm<<endl;
      cout<<"     b      = "<<b<<endl;
...
//===========================the computation body====================================================
   ...
   ADM->Save("Ansorg.psid");
    ...
    {
      const int ext[3] = {1000,1,1};
      const char*filename = "TwPsolution.dat";
      double xmin=-40;
      double xmax= 40;
      double ymin=-40;
      double ymax= 40;
      double zmin=-40;
      double zmax= 40;
      ADM->write_Pablo_file(ext, xmin, xmax, ymin, ymax, zmin, zmax,filename);
      xmin=-8;
      xmax= 8;
      ymin=-8;
      ymax= 8;
      zmin=-8;
      zmax= 8;
      ADM->write_Pablo_file_F(ext, xmin, xmax, ymin, ymax, zmin, zmax,"TwPHam.dat");
    }
